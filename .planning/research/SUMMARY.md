...
- Java 25 LTS · Spring Boot 4.0.6 · Spring AI **2.0.0-M6** behind `LlmGateway`
- spring-cloud-gcp 8.0.2 for **Secret Manager**; Gmail Pub/Sub push receiver stays a plain HTTP controller
- Gradle 9.5.0 + Kotlin DSL · Cloud SQL Postgres 17.6 + Liquibase 5.0.2 (YAML) + JPA · Memorystore Redis 7.2 (cache / session / rate-limit only — not in billing critical path)
...
- Phase 2C — Spring AI 2.0.0-M6 builder/options seams for BYOK model routing + tokenizer strategy
...
Stack: **MEDIUM-HIGH** · Features: **MEDIUM-HIGH** · Architecture: **MEDIUM-HIGH** (Spring AI 2.0.0-M6 BYOK builder/options seams verified during Phase 2C; M6→GA churn remains) · Pitfalls: **HIGH**. **Overall MEDIUM-HIGH — ready to define requirements and generate the roadmap, with one explicit review item around GCP-managed Postgres/Redis lag versus community-latest releases.**
