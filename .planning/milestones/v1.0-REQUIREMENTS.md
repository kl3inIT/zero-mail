...
- [x] **AUTH-03**: User can revoke Gmail access and delete their account + all stored data
- [x] **AUTH-04**: User session persists via cookie-based session (not JWT)
- [x] **AUTH-05**: Disconnected tenants (`invalid_grant`) enter a DISCONNECTED state with user-visible recovery prompt
- [x] **AUTH-06**: User completes guided onboarding (connect Gmail → enable 1 template rule → see first triage preview)

### Foundation & Safety Infrastructure

- [x] **FND-01**: Every request runs in a tenant-scoped context using Scoped Values (never ThreadLocal)
- [x] **FND-02**: ArchUnit test fails any new code that references `ThreadLocal` in request/worker paths
- [x] **FND-03**: `@Sensitive` wrapper and Logback scrub filter prevent email bodies, prompts, or completions from reaching logs
- [x] **FND-04**: ArchUnit test fails any code that references email body / LLM prompt / completion fields in log statements
- [x] **FND-05**: Concurrent multi-tenant integration test confirms no cross-tenant leakage on virtual threads
- [x] **FND-06**: Skeleton OpenAPI spec is published and consumed by the frontend module via `openapi-typescript`
- [x] **FND-07**: CASA restricted-scope verification is initiated at OAuth wiring (not deferred to launch)
...
*Last updated: 2026-05-15 — Milestone v1.0 audit closure: AUTH-03..06 + FND-01..07 flipped to Complete based on Phase 1 plan SUMMARYs (01-01 through 01-09) and downstream PASS evidence (MultiTenantLeakIntegrationTest, LogScrubSyntheticTrafficTest, SafetyContractArchTests, AccountDeletionE2ETest, OnboardingStateMachineTest cited green in 01.1/01.2/01.2.1/02A VERIFICATIONs). 61/61 requirements Complete.*
