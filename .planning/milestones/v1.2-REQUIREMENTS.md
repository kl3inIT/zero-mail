# Requirements Archive: v1.2 Admin Console + User Settings UI
...
- [x] **CAT-01**: Admin can view the catalog at `/admin/catalog` as a per-provider × per-feature matrix. Each cell shows: model display name, model wire ID, enabled toggle (default/recommended badge), cost per 1k input/output tokens (if known), deprecation tag (if applicable), last-synced-at
- [x] **CAT-02**: Admin can trigger Sync-from-`/models` per provider — a 3-step flow: (1) Fetch (async via existing `processing_job` SKIP LOCKED queue, debounced by Redis lease `admin:catalog:sync:<provider>` 60s TTL), (2) Diff (admin reviews additions / removals / metadata changes against the live catalog), (3) Confirm. Auto-apply is forbidden
- [x] **CAT-03**: Catalog Sync uses provider-specific `/models` endpoints (OpenAI-shape: `GET /v1/models`; OpenRouter: `GET /api/v1/models`; DeepSeek: `GET /v1/models`; Google: `GET /v1beta/models`; 9Router: `GET /v1/models`). Anthropic-direct has NO `/models` endpoint — admin enters Anthropic models manually + a Liquibase data seed pre-populates current Claude family. Sync button on the Anthropic-direct provider page is disabled with a tooltip explaining the manual workflow
- [x] **CAT-04**: Sync flow validates every fetched model against a strict per-provider JSON Schema; rejects rows that fail validation; logs rejections to the admin audit. Model IDs must match `^[a-zA-Z0-9._:/\-]{1,128}$` allow-list regex
- [x] **CAT-05**: Admin can soft-delete a model (sets `deprecated_at`, hides from user pickers, keeps existing tenant pins functional); a separate hard-delete flow exists for models with zero dependents only
- [x] **CAT-06**: 9Router-specific combos (entries with `owned_by:"combo"` in `/v1/models`) appear as standalone catalog entries with a "combo" badge; admin curates them like any other model
- [x] **CAT-07**: Admin can bind a model to features (chat / triage / draft) via `feature_binding` table — each (model × feature) pair is independently enabled/disabled/recommended. Foreign key + unique constraint prevent stale pins from `assistant_settings.{chat|triage|draft}_model_id`
...
- [x] **SET-BEHV-03**: User can toggle daily digest (reuses v1.0 ANL-03 config)
...
- [x] **RACT-08**: If an outbound gate fails or the global auto-send setting is OFF, the rule result falls back to Gmail `save_draft` with an audit reason; the system must not silently drop or send the email
...
- [x] **RACT-11**: Low-trust/static sender protections equivalent to Inbox Zero's example-risk guard prevent users from saving demo examples that would send to real people by accident
...
| RACT-08 | Phase 08.1 | Complete |
| RACT-09 | Phase 08.1 | Complete |
| RACT-10 | Phase 08.1 | Complete |
| RACT-11 | Phase 08.1 | Complete |
...
| SET-BEHV-03 | Phase 9 | Complete |
| SET-BEHV-04 | Phase 9 | Complete |
| SET-BEHV-05 | Phase 9 | Deferred (v1.3) |
| SET-SAFE-01 | Phase 9 | Complete |
| SET-SAFE-02 | Phase 9 | Deferred (v1.3) |
| SET-SAFE-03 | Phase 9 | Deferred (v1.3) |
...
*Last updated: 2026-06-01 — v1.2 milestone close. **70/73 requirements complete.** 3 deferred to v1.3: SET-BEHV-05 (surface shadow-mode toggle — only i18n strings shipped, no UI control), SET-SAFE-02 (paste-import) and SET-SAFE-03 (protect/escalate mode) — the latter two deferred at plan-time per Phase 9 success criterion 4. Live outbound-send UAT (08.1-UAT test #6) remains blocked on an external controlled Gmail tenant; automated gates passed.*
