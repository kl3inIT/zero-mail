...
**Confidence:** HIGH for in-repo patterns and locked decisions; MEDIUM for Spring Modulith 2.x events API specifics (snapshot pin, schema-init property name) and Spring AI M6 structured-output behavior; LOW for nothing load-bearing.
...
| Spring AI | 2.0.0-M6 | LLM orchestration (`evaluateSemanticIntents` structured output) | Project-locked; AI-SPEC pre-selects this. `[VERIFIED: gradle/libs.versions.toml]` |
...
| Spring AI 1.x `OutputParser` / ad-hoc JSON | Spring AI 2.x `BeanOutputConverter<T>` + `ResponseFormat.JSON_SCHEMA` strict mode | Spring AI 1.0 → 2.0 (M-series) | The `evaluateSemanticIntents` impl uses strict JSON Schema, not tool-call round-trip — AI-SPEC §3 locks this. Note Spring AI is still on a milestone (M6) — `BeanOutputConverter` may relocate at GA; keep imports in `core.llm.gateway.springai` (ArchUnit-enforced) so a rename is one-file blast radius. `[CITED: 04-AI-SPEC.md §3]` |
...
- **AI:** Spring AI 2.0.0-M6 only; ALL direct Spring AI / vendor SDK usage confined to `core.llm.gateway.springai` (ArchUnit `LlmGatewayBoundaryTest`). `core.triage` calls `LlmGateway` only. `SEMANTIC_INTENT` evaluation MUST go through `LlmGateway.evaluateSemanticIntents(...)`.
...
| A5 | The Spring AI platform model id in `backend/worker/src/main/resources/application.yml` is currently `openai/gpt-4o-mini` (the Phase 2C pin) and Phase 4 raises it to `openai/gpt-5.4-nano` per AI-SPEC §2. Quick task `260508-g41` ("BYOK OpenRouter preset, model selection, Spring AI 2.0.0-M6 sync") may have already changed it. | Runtime State Inventory §Live service config, State of the Art | If the file already says something else, the planner reconciles against the AI-SPEC decision. Read `backend/worker/src/main/resources/application.yml` before authoring the change. |
...
- AI surface (`evaluateSemanticIntents`): **HIGH for the design** (the AI-SPEC pre-locks it down to the entry-point code), **MEDIUM for Spring AI M6 API stability** (milestone — `BeanOutputConverter` may relocate at GA; ArchUnit confines the blast radius).
