...
**Why @Disabled on the test methods (not on the test classes' compile-state):** ArchUnit and `@SpringBootTest` failures cascade through the build at RUN time. `@Disabled("Wave 0 RED scaffold")` keeps each `@Test` method dormant so it cannot fail at run time once the test class eventually compiles. `@Disabled` does NOT affect compilation — disabled tests still compile, so the test files DO break `./gradlew compileTestJava` until the symbols they reference (Plan 03/04/05) land. The frontmatter at the top of this plan is the source of truth on the build-state lifecycle: `compileTestJava` is RED-by-design between Plan 00 and (per-module) Plan 03/04/05; Plan 06 owns the final `./gradlew clean check` GREEN gate. Executors flip `@Disabled → @Test` in Plan 03/04/05 acceptance once the production symbols exist.
  </action>
  <verify>
...
- 1 `@Test @Disabled void amount_mismatch_emits_audit_event_with_vnd_numbers()`:
  - **REVIEWS CYCLE-3 HIGH-3 — fixture corrected.** Cycle-2 corrected the SePay field-order so the service ignores `referenceCode` for lookup and reads `payload.code()` first (with `content` fallback). Cycle-2 ALSO landed the Crockford-alphabet generator (no `I`/`L`/`O`/`U`). The previous fixture used `code="MISMATCH"` which (a) contains the excluded letter `I`, so the service would reject it as malformed and log `event=sepay_unknown_code`, and (b) relied on `referenceCode` to drive lookup, which the service ignores. The corrected fixture uses a valid 8-character Crockford code in `payload.code()`.
  - Seed: create `BillingTopupIntentEntity` with `code="ABCD2345"` (valid 8-char Crockford — alphabet `0-9 A-HJKMNP-TV-Z`, no I/L/O/U), `amountVnd=50000L`, `status=PENDING`. Persist with the standard tenant context.
  - Build payload: same synthetic shape as File 8 (`SepayWebhookPayload` record from Plan 04) BUT with `code="ABCD2345"` (matches the seeded intent — drives the lookup path), `transferAmount=99000L` (intentionally MISMATCHES the seeded `amountVnd=50000`), `referenceCode="BANK-REF-XYZ"` (audit metadata only — must NOT be used for lookup; assert the test still works regardless of this value).
  - POST to `/api/billing/sepay/webhook` with valid `Authorization: Apikey test-sepay-key-fixture` header.
  - Assert: status `200 OK` (per D-C3 — ack to stop SePay retries); body `{"success":true}`; **NO new TOPUP entry was created** (`creditLedgerEntryRepository.count() == 0`); **intent status remains PENDING** (NOT flipped to PAID, finalizable separately by reconciliation).
  ...
  - Assert log buffer DOES NOT contain `event=sepay_unknown_code` (REVIEWS CYCLE-3 HIGH-3 anti-regression: if the fixture drifts back to an invalid Crockford code or to lookup-by-referenceCode, this assertion catches it before the test silently passes the wrong contract).
...
- 1 `@Test @Disabled void insufficient_balance_returns_402_no_balance_leak()`:
  ...
  <verify>
    <automated>find backend/api/src/test/java/com/zeromail/api/controllers/billing -name "*.java" | wc -l returns 8; grep -l "@Disabled" backend/api/src/test/java/com/zeromail/api/controllers/billing/*.java returns all 8; ./gradlew :backend:api:compileTestJava 2>&1 | grep -E "cannot find symbol.*billing" | wc -l > 0 (RED-by-design until Plan 04); grep -q "event=sepay_webhook_amount_mismatch" backend/api/src/test/java/com/zeromail/api/controllers/billing/SepayWebhookMismatchAuditEventTest.java; grep -q 'code = "ABCD2345"\|code="ABCD2345"' backend/api/src/test/java/com/zeromail/api/controllers/billing/SepayWebhookMismatchAuditEventTest.java  # REVIEWS CYCLE-3 HIGH-3: fixture seeds + sends a valid 8-char Crockford code (no I/L/O/U); ! grep -q '"MISMATCH"' backend/api/src/test/java/com/zeromail/api/controllers/billing/SepayWebhookMismatchAuditEventTest.java  # REVIEWS CYCLE-3 HIGH-3: anti-regression — old broken fixture used "MISMATCH" (contains I, excluded by Crockford); grep -q "happy-path log scrub" backend/api/src/test/java/com/zeromail/api/controllers/billing/BillingPrivacyLogScrubTest.java || grep -q "happy path" backend/api/src/test/java/com/zeromail/api/controllers/billing/BillingPrivacyLogScrubTest.java</automated>
  </verify>
  <done>8 api test files exist with `@Disabled` annotations; SepayWebhookIntegrationTest references future SepayWebhookPayload + Authorization Apikey header; BillingBalanceMultiTenantLeakTest mirrors `MultiTenantLeakIntegrationTest` line-by-line on RestClient + LocalServerPort + StructuredTaskScope; BillingPrivacyLogScrubTest negative-asserts on `Apikey ` + `accountNumber` + `bank sms` substrings AND its Javadoc explicitly limits scope to happy path; SepayWebhookMismatchAuditEventTest positively asserts `event=sepay_webhook_amount_mismatch` + `intentVnd=` + `actualVnd=` strings in log output (D-I1 dedicated coverage) — REVIEWS CYCLE-3 HIGH-3: fixture seeds intent with valid 8-char Crockford `code="ABCD2345"` and sends webhook payload with `code="ABCD2345"` + mismatching `transferAmount`; the old broken `code="MISMATCH"` (containing excluded letter `I`) is gone and a negative grep guards regression.</done>
</task>
