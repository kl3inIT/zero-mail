...
must_haves:
  truths:
    - "POST /api/llm/byok/validate accepts {provider, endpoint?, apiKey} (API DTO ByokValidateRequest) and ByokController maps to a core command record ByokValidateCommand; ByokService.validate() takes the core command (NOT the API DTO) — REVIEWS HIGH-consensus #3 layering fix. (HIGH-3 cycle-3) Server-side probe URL is `${canonicalEndpoint}/models` for openai-compatible (NEVER `${endpoint}/v1/models` — would double-prefix `/api/v1/v1/models` for OpenRouter) or `${canonicalEndpoint}/messages` (max_tokens=1) for anthropic; the browser NEVER issues the validate call directly"
...
- OpenAI-compatible: `GET ${canonicalEndpoint}/models` with `Authorization: Bearer ${apiKey}` → 200 + `{ data: [{id: "..."}, ...] }`. For `https://openrouter.ai/api/v1` the URL is `https://openrouter.ai/api/v1/models`. For `https://api.openai.com/v1` the URL is `https://api.openai.com/v1/models`.
- Anthropic: `POST ${canonicalEndpoint}/messages` with `x-api-key: ${apiKey}` + `anthropic-version: 2023-06-01` body `{model, max_tokens: 1, messages: [{role: "user", content: "."}]}` → 200 = valid. Default canonical Anthropic endpoint is `https://api.anthropic.com/v1`; URL is `https://api.anthropic.com/v1/messages`.
</interfaces>
</context>
...
<task type="auto" tdd="true">
  ...
  <behavior>
    - Test 1 (ByokServiceTest#validate_openai_compatible_calls_models_endpoint): **(HIGH-3 cycle-3)** input endpoint is `https://together.xyz/v1`; WireMock stub at `https://together.xyz/v1/models` (NOT `/v1/v1/models`) returning 200 with `{data: [{id: "model-a"}, {id: "model-b"}]}`; ByokService.validate returns `ByokValidateResult(ok=true, models=["model-a","model-b"], reason=null)`. Capture the actual outbound URL via WireMock `findAll(getRequestedFor(...))` and assert it equals exactly `https://together.xyz/v1/models`. (Operator opt-in flag must be `true` for `together.xyz` to pass the validator — set in test.)
    - Test 2 (ByokServiceTest#validate_openai_compatible_failure): WireMock returns 401 → returns `ByokValidateResult(ok=false, models=null, reason="upstream_rejected")` — reason is opaque (no upstream body bytes).
    - Test 3 (ByokServiceTest#validate_anthropic_calls_messages_endpoint): **(HIGH-3 cycle-3)** input endpoint is `https://api.anthropic.com/v1` (or null → default canonical includes `/v1`); WireMock at `/v1/messages` POST with `max_tokens=1` body returning 200 → returns `ByokValidateResult(ok=true, models=null, reason=null)`. Captured outbound URL is exactly `https://api.anthropic.com/v1/messages`.
    - Test 3a (HIGH-3 cycle-3 regression — `ByokServiceTest#openrouter_validate_does_not_double_prefix_v1`): input endpoint `https://openrouter.ai/api/v1`; capture outbound URL via WireMock; assert URL equals exactly `https://openrouter.ai/api/v1/models` (single `/v1`). Assert URL does NOT match `.*\/v1\/v1\/.*` regex. This is the exact failure mode cycle-2 surfaced.
    - Test 3b (HIGH-3 cycle-3 regression — `ByokServiceTest#openai_validate_uses_v1_models`): input endpoint `https://api.openai.com/v1`; assert outbound URL equals exactly `https://api.openai.com/v1/models`.
    - Test 3c (HIGH-3 cycle-3 regression — `ByokServiceTest#trailing_slash_does_not_change_outbound_url`): input endpoint `https://openrouter.ai/api/v1/` (trailing slash); assert outbound URL equals exactly `https://openrouter.ai/api/v1/models` (validator strips trailing `/`).
    ...
    - Test 19 (HIGH-2 cycle-3 — `ByokControllerIntegrationTest#post_validate_accepts_lowercase_provider_id`): POST `{"provider":"openai-compatible","endpoint":"https://openrouter.ai/api/v1","apiKey":"k"}` to `/api/llm/byok/validate` returns 200 (assuming WireMock upstream stub). The deserialized `ByokValidateRequest` carries `BYOKProvider.OPENAI_COMPATIBLE`. POST `{"provider":"OPENAI_COMPATIBLE",...}` returns 400 (the constant name is NOT a valid id; @JsonCreator delegates to `fromId(String)` which fail-louds).
    - Test 20 (HIGH-2 cycle-3 — `ByokControllerIntegrationTest#get_current_returns_lowercase_provider_id`): GET `/api/llm/byok` for a tenant with a saved Anthropic BYOK row returns body containing `"provider":"anthropic"` (NOT `"ANTHROPIC"`). Assert via JsonPath `$.provider`.
  </behavior>
  <action>
    ...
    2. **Create `backend/core/src/main/java/com/zeromail/core/llm/service/ByokService.java`** — `@Service`. Methods (REVIEWS HIGH-consensus #3 — accept core commands only):
       - `validate(UUID tenantId, ByokValidateCommand command)`:
         ...
         - Issues outbound HTTP via Spring `RestClient` (see `BillingTopupService` for analog). Branch on provider. **(HIGH-3 cycle-3 lock)** The canonical endpoint already includes the API version path; appending `/v1/...` here would produce `/api/v1/v1/models` for OpenRouter and `/v1/v1/messages` for Anthropic. Append ONLY `/models` or `/messages`:
           - OPENAI_COMPATIBLE → `GET ${canonicalEndpoint}/models` with `Authorization: Bearer ${command.apiKey()}`. On 200 → parse → `ByokValidateResult(true, modelIds, null)`. On non-2xx → `ByokValidateResult(false, null, "upstream_rejected" | "connection_failed")`.
           - ANTHROPIC → `POST ${canonicalEndpoint}/messages` with `x-api-key`, `anthropic-version: 2023-06-01`, body `{model: "claude-3-haiku-20240307", max_tokens: 1, messages: [{role: "user", content: "."}]}`. On 200 → ok=true.
         - Centralize the URL-join in a private helper: `private String joinPath(String canonicalEndpoint, String suffix) { return canonicalEndpoint.replaceAll("/+$", "") + "/" + suffix.replaceAll("^/+", ""); }`. Use this helper EXACTLY once per outbound call (validate + save re-probe both go through it). Acceptance grep: `grep -c "joinPath" backend/core/src/main/java/com/zeromail/core/llm/service/ByokService.java` returns `>= 2` (validate + save re-probe).
  ...
  <acceptance_criteria>
    ...
    - `./gradlew :backend:core:test --tests "ByokServiceTest"` exits 0 — including H-4 tests (7–10) AND HIGH-3 cycle-3 regression tests (3a `openrouter_validate_does_not_double_prefix_v1`, 3b `openai_validate_uses_v1_models`, 3c `trailing_slash_does_not_change_outbound_url`).
    - **(HIGH-3 cycle-3)** `grep -c '/v1/models\|/v1/messages' backend/core/src/main/java/com/zeromail/core/llm/service/ByokService.java` returns `0` (validate uses `/models` and `/messages` only — canonical endpoint already includes `/v1`).
    - **(HIGH-3 cycle-3)** `grep -c 'joinPath' backend/core/src/main/java/com/zeromail/core/llm/service/ByokService.java` returns `>= 2` (centralized URL-join helper used by validate + save re-probe).
    - **(HIGH-2 cycle-3)** Plan 05b adds a controller-integration JSON test in `ByokControllerIntegrationTest`: `post_validate_accepts_lowercase_provider_id` — POST body `{"provider":"openai-compatible","endpoint":"https://openrouter.ai/api/v1","apiKey":"k"}` deserializes correctly (Jackson @JsonCreator on BYOKProvider). Asserts the parsed command has `provider == BYOKProvider.OPENAI_COMPATIBLE`. The reverse (response-side `BYOKProvider` serializing to lowercase id) is asserted by `get_current_returns_lowercase_provider_id`.
