...
- Query: atomically claim rows with `UPDATE ... WHERE id IN (SELECT ... FROM pubsub_delivery WHERE (status='PENDING' AND (locked_until IS NULL OR locked_until < NOW())) OR (status='PROCESSING' AND locked_until < NOW()) FOR UPDATE SKIP LOCKED LIMIT N) RETURNING *`.
- Per row: bind `TenantContext.TENANT`, refresh access token via `RefreshTokenCipher` decrypt + Google OAuth refresh, call `gmail.users().history().list(startHistoryId=last_synced_history_id, historyTypes=[messageAdded], labelId=INBOX, maxResults=500)`.
...
- For each candidate `messagesAdded` entry: call `gmail.users().messages().get(userId='me', id, format='metadata', fields='id,threadId,labelIds,internalDate')`, then emit only when the metadata `labelIds` includes `INBOX`: `INSERT INTO mail_message_observed (tenant_id, gmail_message_id, gmail_thread_id, history_id, label_ids, internal_date, observed_at) ON CONFLICT (tenant_id, gmail_message_id) DO NOTHING`. Privacy: NO subject, NO from, NO body, NO snippet, NO headers — only IDs + label snapshot + internal date.
...
- Persist response: `watch_history_id` (returned baseline), `watch_expires_at` (returned `expiration` ms epoch), `watch_renewed_at = NOW()`. Set `ingestion_health = HEALTHY` only if the current value is `WATCH_UNHEALTHY` and watch succeeded. Preserve `HISTORY_LOST` until the explicit reconnect/re-consent path calls `clearForReconnect`, so the reconnect prompt remains visible. Only initialize `last_synced_history_id` from the returned baseline when it is NULL (initial registration or reconnect); regular renewal must preserve a non-null cursor so queued/unprocessed history is not skipped.
...
- **D-B4: MessageObserved emit policy = `messagesAdded` only, INBOX-filtered.** Worker calls `history.list` with `historyTypes=['messageAdded']` and `labelId='INBOX'`, then treats returned message objects as identifiers only. For each candidate ID it calls `messages.get(format='metadata', fields='id,threadId,labelIds,internalDate')`, emits `mail_message_observed` ONLY when the metadata `labelIds` includes `INBOX`, and stores only IDs + label snapshot + internal date. Skips `labelsAdded` / `labelsRemoved` events (Inbox-zero processes these for label-change reactions; v1 doesn't need them). Defer label-change observation to Phase 4 if rule engine surfaces a need (cheap forward addition: same table, new code path).
...
- **D-D4: Reconnect handler clears `last_synced_history_id` + `watch_*` columns to force re-register.** After a successful explicit reconnect/re-consent via `/tenant/connect-gmail`, the bundled-OAuth success handler (or its post-success cleanup) sets `last_synced_history_id = NULL`, `watch_expires_at = NULL`, `watch_history_id = NULL`, `watch_consecutive_failures = 0`, `ingestion_health = HEALTHY`. Worker `GmailWatchScheduler` picks up the row on its next minute-tick (NULL expiry matches the initial-register condition) → re-issues `users.watch` → initializes a fresh sync baseline because the cursor is NULL → ingress resumes normally. Ordinary login/upsert does not call this cleanup. Regular watch renewal never clears or advances a non-null `last_synced_history_id`, and never clears `HISTORY_LOST`.
...
- **Spring MVC OIDC verification filter** — implement as a non-`@Component` Spring `OncePerRequestFilter` added only to a dedicated `SecurityFilterChain @Bean @Order(1)` with `securityMatcher("/internal/pubsub/**")`. Put `@Order` on the filter-chain bean, not the configuration class. The chain runs before the user-OAuth chain and uses `permitAll` after the Pub/Sub OIDC filter authenticates the request.
...
- `backend/api/src/main/java/com/zeromail/api/security/PubSubSecurityConfig.java` — add new `SecurityFilterChain @Bean @Order(1)` for `/internal/pubsub/**` matcher with `permitAll` + custom OIDC verification filter. `backend/api/src/main/java/com/zeromail/api/security/SecurityConfig.java` keeps the user OAuth chain with `@Order(2)` on its `SecurityFilterChain` bean.
...
- `backend/core/src/main/java/com/zeromail/core/gmail/service/GmailConnectionService.java` — extend `disconnect(...)` to call `users.stop()` and null watch_* columns; add `markHistoryLost(...)`, `markWatchUnhealthy(...)`, `clearForReconnect(...)`, `recordWatchSuccess(...)` methods. `recordWatchSuccess` initializes `last_synced_history_id` only when NULL, clears only `WATCH_UNHEALTHY` on success, and preserves `HISTORY_LOST`; renewal never advances a non-null cursor.
...
- **Google Gmail API v1** — `users.watch`, `users.stop`, `users.history.list` (params: `startHistoryId`, `historyTypes=['messageAdded']`, `maxResults=500`, `labelId=INBOX`), `users.messages.get(format=metadata, fields=id,threadId,labelIds,internalDate)` for label/internalDate metadata only, error codes (404 historyId expired, 401 invalid_grant, 403 quota).
...
- **`GmailHistoryProcessor` ↔ `mail_message_observed`** — `messages.get(format=metadata, fields=id,threadId,labelIds,internalDate)` per history candidate, then INSERT ON CONFLICT DO NOTHING only when metadata labels include INBOX.
...
- **Worker idempotency under crashes**: `pubsub_delivery.status` advances PENDING → PROCESSING during atomic claim, then PROCESSING → PROCESSED only after `mail_message_observed` writes commit + `gmail_connections.last_synced_history_id` updates commit. Single transaction for per-delivery fan-out. If worker crashes after claim or mid-fan-out, `locked_until` eventually expires and `claimPendingBatch` reclaims the stale PROCESSING row; `mail_message_observed` ON CONFLICT skips already-written rows, `last_synced_history_id` monotonic-conditional UPDATE skips no-op. Net: at-least-once worker semantics + idempotent observation = exactly-once observation.
