...
<task type="auto" tdd="true">
  ...
  <files>
    ...
    apps/admin/index.html,
    apps/admin/src/main.tsx,
    apps/admin/src/routeTree.gen.ts (auto-generated by @tanstack/router-plugin — gitignored, regenerated on dev/build),
    apps/admin/src/lib/api/admin-client.ts,
    apps/admin/src/lib/api/admin-base-url.ts,
    apps/admin/scripts/generate-api.ts,
    apps/admin/src/lib/webauthn.ts,
    apps/admin/src/components/ui/,
    apps/admin/src/components/AdminModeBanner.tsx,
    ...
    apps/admin/src/routes/_authenticated/index.tsx,
    apps/admin/src/routes/_authenticated/audit.tsx,
    apps/admin/src/routes/_authenticated/role-grants.tsx,
    ...
    apps/admin/playwright.config.ts,
    apps/admin/.gitignore (entries: dist/, node_modules/, src/routeTree.gen.ts),
  ...
  <read_first>
    ...
    .planning/phases/08-admin-console-operator-tooling/08-CONTEXT.md §D-24 (apps/admin stack version lock — EXACT versions, file-based routing convention, TanStack Router plugin ordering),
    CONVENTIONS.md §Frontend feature API/hooks/query keys,
    Context7 `/tanstack/router` — Vite plugin setup, file-based routing conventions, validateSearch with Zod, useSearch hook, code-splitting via autoCodeSplitting,
    Context7 `/tanstack/form` — useForm with Standard Schema (Zod 4) validators, form.Field children render-prop, onChange + onChangeAsync + onChangeAsyncDebounceMs, integration with TanStack Query mutation onSubmit
  </read_first>
  <behavior>
    - `apps/admin/package.json` declares `@zeromail/admin` workspace with EXACT versions locked per CONTEXT.md §D-24 (verified live on npm registry 2026-05-20). dependencies: `react@19.2.6`, `react-dom@19.2.6`, `@tanstack/react-router@1.170.4`, `@tanstack/react-query@5.100.11`, `@tanstack/react-form@1.32.0`, `zod@4.4.3`, `openapi-fetch@0.17.0`, `@simplewebauthn/browser@13.3.0`, `lucide-react@latest`, `sonner@^2`, `class-variance-authority`, `clsx`, `tailwind-merge`. devDependencies: `vite@8.0.13`, `@vitejs/plugin-react-swc@4.3.1`, `@tanstack/router-plugin@1.168.6`, `@tanstack/router-devtools@1.167.0`, `@tanstack/react-query-devtools@5.100.11`, `typescript@6.0.3`, `tailwindcss@4.3.0`, `@tailwindcss/vite@4.3.0`, `openapi-typescript@7.13.0`, `tsx@^4`, `vitest@4.1.6`, `@vitest/ui@4.1.6`, `@playwright/test@1.60.0`, `@types/react@^19`, `@types/react-dom@^19`. **DO NOT install `@tanstack/zod-form-adapter`** — deprecated; Zod 4 implements Standard Schema 1.0 natively and TanStack Form v1 accepts it directly via `validators: { onChange: schema }`. **NO `react-router-dom`** — TanStack Router replaces it (file-based routing). **NO `tailwind.config.ts`, NO `postcss.config.js`** — Tailwind 4 reads `@theme` block in `globals.css`; `@tailwindcss/vite` plugin handles PostCSS internally.
    ...
    - `apps/admin/vite.config.ts` plugin order is CRITICAL (Context7-verified silent failure if wrong): `[tanstackRouter({ target: 'react', autoCodeSplitting: true }), react(), tailwindcss()]` — `tanstackRouter()` MUST come BEFORE `react()`. Routes are auto-discovered from `src/routes/**`; `routeTree.gen.ts` regenerated on each `pnpm dev` / `pnpm build` and gitignored.
    - File-based routing convention (per CONTEXT.md §D-24): `src/routes/__root.tsx` defines `createRootRoute({ component })` with `<Outlet />`; `src/routes/_authenticated.tsx` is a layout route enforcing WebAuthn session (calls `GET /api/admin/me` → on 401 redirects to `/login`); routes inside `_authenticated/` inherit the layout (AdminLayout + AdminModeBanner) and the auth gate. Dynamic params use `$` prefix (e.g., `_authenticated/tenants.$tenantId.tsx`); query params validated via `validateSearch: zodSchema` and read via `Route.useSearch()` with full type inference.
    ...
    - `apps/admin/src/lib/webauthn.ts` wraps `@simplewebauthn/browser@13.3.0` `startRegistration({optionsJSON})` and `startAuthentication({optionsJSON})` (v13 wrapper shape per Context7); typed inputs from admin-schema.d.ts WebAuthn endpoint responses.
    ...
    - `/enroll?token={hex}` route (`src/routes/enroll.tsx`): `validateSearch: z.object({ token: z.string().length(64) })`; reads token via `Route.useSearch()`; renders centered card "Register passkey for {email}" with TanStack Form (`useForm` + Zod `z.object({ email: z.email() })` passed directly as `validators.onChange`); on submit calls `webauthn.ts` startRegistration → on success router.navigate to `/login`.
    - `/login` route (`src/routes/login.tsx`): TanStack Form `email` field + `Sign in with passkey` button → startAuthentication → on success router.navigate to `/`.
    - `_authenticated/index.tsx` (dashboard, mounted at `/`): KpiCard stubs + recent-audit list (queries `/api/admin/audit/events?limit=10` via TanStack Query).
    - `_authenticated/audit.tsx`: filter bar (actor/action/target/date) using TanStack Form for filter state + paginated table with inline `<JsonDiffViewer>` expand on row click + CSV export button (streams from `/api/admin/audit/events/csv`). Filter state synced to URL via `validateSearch` + `Route.useSearch()` so admin can share filtered audit URLs with co-admin.
    - `_authenticated/role-grants.tsx`: admin list table + `Grant admin` dialog with TanStack Form (`useForm` + Zod `z.object({ email: z.email(), reason: z.string().min(8).max(500) })` as validators) → on submit calls `POST /api/admin/grant-admin` via TanStack Query mutation → result modal showing one-time enrollment URL with `Copy URL` button. `Revoke admin` button per row uses `<ConfirmTwiceDialog>` with `reason` form (TanStack Form) + admin email as step-2 token.
    - Feature folders per CONVENTIONS §8: `features/audit/{api,query-keys,use-audit-page}`, `features/role-grants/{api,query-keys,use-admins,use-grant-admin,use-revoke-admin}`. Use generated types from `admin-schema.d.ts`, NEVER hand-write mirror DTOs. Use `api.GET`/`api.POST` typed client, NOT raw fetch. Forms use TanStack Form `useForm` + Zod 4 schema as Standard Schema validator — NEVER react-hook-form, NEVER `@tanstack/zod-form-adapter` (deprecated).
  ...
  <action>
    Pin EXACT versions per CONTEXT.md §D-24 in `apps/admin/package.json` (verified npm registry 2026-05-20: vite 8.0.13, @tanstack/react-router 1.170.4, @tanstack/react-form 1.32.0, zod 4.4.3, react 19.2.6, typescript 6.0.3, tailwindcss 4.3.0, @simplewebauthn/browser 13.3.0, vitest 4.1.6, @playwright/test 1.60.0 — full list in D-24). Run `pnpm dlx shadcn@latest init` inside `apps/admin` with `base-nova` preset to seed components.json + globals.css base, then copy each primitive from apps/web/components/ui/ byte-identical (per UI-SPEC §Component Inventory list — 24 primitives). Extend globals.css with `@theme` block containing `.zm-proto` palette tokens + `--warning-soft: #FDE8BA`, `--amber: #E3A023`, `--ink-2: #4F496B`, `--ring: #867AEB` (Tailwind 4 CSS-only config; NO `tailwind.config.ts`, NO `postcss.config.js`). AdminModeBanner copy is locked verbatim per UI-SPEC §Copywriting line 124. ConfirmTwiceDialog props: `{open, onOpenChange, actionLabel, targetLabel, consequences: string[], confirmationToken: string, finalButtonLabel, variant?: 'destructive'|'warning', onConfirm(reason): Promise<{auditId:string}>}` — internal form state via TanStack Form `useForm` with Zod 4 schema `z.object({ reason: z.string().min(8).max(500).refine(noSentinels) })`. WebAuthn ceremonies via Context7-confirmed `@simplewebauthn/browser@13.3.0` `startRegistration({optionsJSON})` shape. Generate admin-schema.d.ts via `pnpm --filter @zeromail/admin generate-api` (run after backend boots with /v3/api-docs/admin available). TanStack Router file-based routing: `src/routes/__root.tsx` createRootRoute → `_authenticated.tsx` layout with WebAuthn session gate → child routes inherit AdminLayout + AdminModeBanner; query params validated via `validateSearch: zodSchema` for type-safe `Route.useSearch()`. `vite.config.ts` plugin order CRITICAL: `tanstackRouter()` MUST come before `react()` (Context7-verified silent failure). Playwright virtual authenticator setup per Context7 `/microsoft/playwright` "WebAuthn" guide. ESLint cross-workspace block: pattern `**/apps/admin/**` with message "apps/web cannot import from apps/admin — admin schema types stay out of public bundle (ADMIN-06)". DO NOT install `react-router-dom`, DO NOT install `react-hook-form`, DO NOT install `@tanstack/zod-form-adapter` (deprecated).
  </action>
  <verify>
  ...
  <acceptance_criteria>
    ...
    - Version pins verified: `jq -r '.dependencies["@tanstack/react-router"]' apps/admin/package.json` equals `1.170.4`; same for `@tanstack/react-form` (`1.32.0`), `zod` (`4.4.3`), `@simplewebauthn/browser` (`13.3.0`); `jq -r '.devDependencies["vite"]' apps/admin/package.json` equals `8.0.13`; `jq -r '.devDependencies["typescript"]' apps/admin/package.json` equals `6.0.3`; `jq -r '.devDependencies["tailwindcss"]' apps/admin/package.json` equals `4.3.0`.
    - Banned packages absent: `jq -r '.dependencies | keys[], .devDependencies | keys[]' apps/admin/package.json | grep -E '^(react-router-dom|react-hook-form|@tanstack/zod-form-adapter)$'` returns empty (zero matches).
    - Plugin ordering verified: `grep -nE 'tanstackRouter|react\(\)' apps/admin/vite.config.ts | head -2 | awk -F: '{print $2}' | sort -n | head -1` returns a line containing `tanstackRouter` (i.e., tanstackRouter listed before react in plugins array).
    - File-based routing artifacts present: `test -f apps/admin/src/routes/__root.tsx`, `test -f apps/admin/src/routes/_authenticated.tsx`, `test -f apps/admin/src/routes/enroll.tsx`, `test -f apps/admin/src/routes/login.tsx`, `test -f apps/admin/src/routes/_authenticated/index.tsx`, `test -f apps/admin/src/routes/_authenticated/audit.tsx`, `test -f apps/admin/src/routes/_authenticated/role-grants.tsx` all exit 0. `apps/admin/src/routeTree.gen.ts` is gitignored (`grep -q routeTree.gen.ts apps/admin/.gitignore`).
    - Tailwind 4 CSS-only config verified: `apps/admin/tailwind.config.ts` does NOT exist; `apps/admin/postcss.config.js` does NOT exist; `grep -q '@theme' apps/admin/src/styles/globals.css` exits 0.
    ...
    - Vitest unit test on `<ConfirmTwiceDialog>` (using TanStack Form internally): typing reason `key:sk-test123` shows inline error from Zod refine; reason `compromised hardware` enables `Continue`; second-step token mismatch keeps final button disabled.
