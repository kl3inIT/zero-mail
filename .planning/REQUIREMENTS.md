...
- [ ] **CAL-TRIAGE-03**: Calendar-class messages are guarded against destructive rule actions (`archive`, `mark_spam`, `delete`) — the rule engine logs a `CalendarAwareGuard` audit when a rule would have acted destructively, and the action is downgraded to `label` only.
