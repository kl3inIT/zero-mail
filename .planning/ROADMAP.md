...
- [ ] **Phase 2B: Billing (Prepaid Credits)** - Double-entry Postgres ledger, reserve/settle/release, credit-hold watchdog, SePay/VietQR top-up intent + webhook, balance API hooks
...
Plans:
- [ ] 02B-00-wave0-tests-PLAN.md — Wave 0 RED test scaffolds (7 core + 7 api + 2 worker = 16 files; flip VALIDATION nyquist_compliant true)
- [ ] 02B-01-schema-and-deps-PLAN.md — Liquibase 014 (credit_ledger_entry) + 015 (credit_reservation) + 016 (billing_topup_intent) + 017 (shedlock); ShedLock 7.7.0 + worker build wiring
- [ ] 02B-02-domain-model-PLAN.md — core.billing Modulith leaf + model package (CreditLedger interface + 3 enums + 2 records + 2 exceptions; BYOK Javadoc clause)
...
| 2B. Billing (Prepaid Credits) | 0/7 | Planned | - |
