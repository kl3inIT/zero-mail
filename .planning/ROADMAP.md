...
Plans:
- [x] 05B-00-PLAN.md — [BLOCKING] Wave 0: jakarta.mail dep (+ no `jakarta.activation` skew) + `030-thread-reply-status` Liquibase changelog (composite inbox keyset index + partial TO_REPLY count index + FK `ON DELETE CASCADE` as the sole cleanup) + 14 RED test scaffolds, FQN/reflection style only so `compileTestJava`/`tsc` stay GREEN (incl. `TriageAuditSagaDraftThreadingTest`, `AutomaticTriageDraftUsesToneGenerationTest`) + ArchUnit guards + EN_SCAN_FILES + public bucket slugs `to-reply`/`awaiting-their-reply`
- [x] 05B-01-PLAN.md — Threading retrofit on the REAL draft-write path (`TriageOrchestratorService → TriageAuditSaga.gmailWritePhase → TriageGmailWriter.saveDraft`): `jakarta.mail` `MimeMessage` reply builder + `ReplyHeaders` (carried on `TriageAuditCommand`, required for `SaveDraft`) + `ThreadingHeaderValidator` (fail-closed → `GmailWriteResult.failed`) + extend the inbound message metadata (`GmailPreviewMessage` + `TriageRuleEvaluationInputFactory`) with `Message-ID`/`References`/`Reply-To` + `TriageOrchestratorService` wiring; localized `Re:`-prefix doubling documented as a v1 cosmetic limit
- [x] 05B-02-PLAN.md — `core.thread` package: `ThreadReplyBucket` IdentifiedEnum (+ public-slug helpers) + `thread_reply_status` entity/repo/converter (no `deleteByTenantId` — FK cascade) + heuristic `ClassifyThreadReplyStatusService` (public `classify(...)` + Modulith reactions on `ThreadDraftSaved` and `MailOutboundObserved`) + the `MailOutboundObserved` event + its publish from `GmailDeliveryProcessingService` (SENT-labelled observed mail) — awaiting-reply is bounded by observed SENT mail (no `messages.list`/`in:sent`), a documented limit; does NOT edit `TriageOrchestratorService`/`TriageAuditSaga` (Plan 03 owns the inbound sub-step + `triage → thread` edge; Plan 01 owns the saga)
- [x] 05B-03-PLAN.md — `core.draft` package: `ToneContextBuilder` (in-request sent-mail fetch + strip + sanitize; degrades to descriptors-only on token-budget OR ANY Gmail-API failure) + `DraftBodyGenerator` (LLM-only body, no Gmail/DB write, used by BOTH the on-demand service AND the triage orchestrator → DRFT-03 on the primary automatic path) + `GenerateThreadDraftService` (Redis lock → **save NEW draft then delete OLD** → persist + upsert + publish `ThreadDraftSaved`; no DB tx spans the LLM or any Gmail call) + `RedisDistributedLock` + `LlmGateway.chatForDraft` seam taking ONLY `core.llm`/primitive inputs (no `core.draft` type → no `llm → draft` cycle) + `SAVE_DRAFT_ONLY` profile (`CallSite.DRAFT` reused) + the `TriageOrchestratorService` body-generation + `classify(...)` sub-step + `core/triage/package-info.java` `triage → thread`/`triage → draft` edges + the `shared.pagination` edge added to `core/triage/package-info.java` AND `core/thread/package-info.java` (on behalf of 05B-04 projection sub-packages); does NOT re-edit `TriageAuditSaga.java`
- [x] 05B-04-PLAN.md — CQRS-lite read side (`depends_on: 05B-02, 05B-03` — NOT parallel with 03): `AuditLogQueryService` (closes the 5A `GET /api/triage/audit` gap) + `NeedsReplyInboxQueryService` (NULLS-LAST keyset over the string `gmail_thread_id`, index-driven by `idx_thread_reply_status_inbox`) + `toReplyCount` + `KeysetCursor` codec (full-`Instant` encoding, distinct `nullsLast()` token — never `Instant.EPOCH`) + `MarkThreadResolvedService`; does NOT touch the parent `package-info.java` files (Plan 03 owns those edges)
- [x] 05B-05-PLAN.md — backend/api: `GET /api/triage/audit` + two disjoint `/api/threads` controllers (`ThreadDraftController` `@Tag("thread-draft")` `POST /{id}/draft` (409 on contention) + `POST .../resolve`; `NeedsReplyInboxController` `@Tag("thread-inbox")` `GET /api/threads?bucket=to-reply|awaiting-their-reply` with ONE `BatchRequest` of `threads.get(METADATA)` per page — Gmail-quota note carried into the SUMMARY) + DTOs + `GlobalExceptionHandler`/`ErrorCodes` (`INVALID_CURSOR` 400 via a typed `InvalidCursorException`; `DRAFT_GENERATION_FAILED`/`LLM_SAFETY_VIOLATION` → 422, not 500) + vi/en error i18n + OpenAPI regen
- [x] 05B-06-PLAN.md — apps/web: `features/needs-reply/` (API/keys + 4 hooks incl. a cheap dedicated `useToReplyCount` for the sidebar badge, so a route change never triggers the per-row Gmail fan-out + components) + `/needs-reply` route + "Needs reply" sidebar item + `TO_REPLY` badge + "Draft reply/Regenerate draft" action (inbox + now-live audit rows; 409 → inline notice + ~3-5s button cooldown; regenerate gated behind an alert-dialog confirm) + remove 5A GAP sentinels + i18n (lock-step vi/en; safety-related vi copy flagged for human review); component/e2e tests use MSW/API fixtures
- [x] 05B-07-PLAN.md — closure: `:backend:core:aiEval` source set + a CI job invoking it + deterministic eval dims (4 safety, 6 threading, 8 token budget pass unconditionally; **dim 7 classifier ≥85% with ≥5 edge-case fixtures GATES the merge and routes DRFT-04's status — never "measure and document with the bar intact", never disabled**) + synthetic fixtures (≥15 draft; 20-30 classifier, ≥5 edge cases) + AI-SPEC updated to `CallSite.DRAFT` (no `DRAFT_REPLY`) + `DraftPrivacySweepTest` (no content leak via logs OR exceptions, success AND failure paths) + full `check` + apps/web gates + `05B-VALIDATION.md`/`05B-UAT.md` + flip DRFT-01..03 Complete, DRFT-04 Complete-or-Partially-complete + ROADMAP

**Waves & parallelization**:
- Wave 0: `05B-00` (blocking — schema + jakarta.mail dep + RED scaffolds, FQN/reflection style so `compileTestJava` stays green).
- Wave 2 (after Wave 0): `05B-01` and `05B-02` run in parallel — disjoint `files_modified` (`05B-01` owns the `core.triage` threading + the REAL draft-write path: `TriageGmailWriter` + `ReplyHeaders` + `ThreadingHeaderValidator` + `TriageAuditSaga` (`TriageAuditCommand`/`gmailWritePhase`) + `TriageRuleEvaluationInputFactory` + `GmailPreviewReadService` metadata extension + the `TriageOrchestratorService` `ReplyHeaders` build; `05B-02` owns `core.thread` + `core.gmail.event.MailOutboundObserved` + `GmailDeliveryProcessingService` and does NOT touch `TriageOrchestratorService.java` or `TriageAuditSaga.java`). The `TriageOrchestratorService.java` body-generation + `classify(...)` sub-step and the `triage → thread`/`triage → draft` Modulith edges live in exactly one plan: `05B-03`, which depends on both `05B-01` and `05B-02`.
- Wave 3 (after Wave 2): `05B-03` (`depends_on: [05B-01, 05B-02]`) — `core.draft` + `core.shared.lock` + the `LlmGateway.chatForDraft` neutral seam + the `core.triage` orchestrator sub-step + the two parent `package-info.java` files `core/triage` and `core/thread`.
- Wave 4 (after Wave 3): `05B-04` (`depends_on: [05B-02, 05B-03]` — NOT parallel with 03, so 03's parent-`package-info.java` edits are committed first) — `core.*.projection` + the new `core.shared.pagination` leaf module + `MarkThreadResolvedService`; `05B-04` does NOT touch `core/triage/package-info.java` or `core/thread/package-info.java` (Plan 03 owns the `shared.pagination` edge on those parents).
- Wave 5 (after Wave 4): `05B-05` (`depends_on: [05B-03, 05B-04]`) — backend/api wiring + OpenAPI regen.
- Wave 6: `05B-06` (apps/web UI, `depends_on: [05B-05]`).
- Wave 7: `05B-07` (closure, `depends_on: [05B-01..06]`).
...
**Plans**: 4 plans
- [ ] 05C-01-PLAN.md — Schema + entity wave: sender_email column (§0 fix), tenants.time_zone, notification_preference + digest_delivery tables + analytics indexes, JPA entities + IdentifiedEnum ChannelType/DigestDeliveryStatus, OAuthProvisioningService defaults + account-deletion cascade
- [ ] 05C-02-PLAN.md — Backend analytics endpoint: core.analytics Modulith module, AnalyticsSummaryQueryService (Q1-Q4 JDBC), TimeSavedWeights, AnalyticsController GET /api/analytics/summary, content-ban + privacy-sweep ArchUnit tests
- [ ] 05C-03-PLAN.md — Backend digest dispatch: Resend SDK 4.13.0, Thymeleaf vi/en templates, EmailNotificationChannel + ResendEmailGateway + ThymeleafDigestRenderer, DigestDispatchScheduler (cron + ShedLock), DigestPendingReaperJob, NotificationPreferencesController GET/PATCH, ResendBoundaryArchTest + DigestPayloadShapeArchTest + privacy sweep + idempotency tests
