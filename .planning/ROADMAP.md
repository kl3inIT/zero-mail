...
- [ ] 05B-04-PLAN.md — CQRS-lite read side (`depends_on: 05B-02, 05B-03` — NOT parallel with 03): `AuditLogQueryService` (closes the 5A `GET /api/triage/audit` gap) + `NeedsReplyInboxQueryService` (NULLS-LAST keyset over the string `gmail_thread_id`, index-driven by `idx_thread_reply_status_inbox`) + `toReplyCount` + `KeysetCursor` codec (full-`Instant` encoding, distinct `nullsLast()` token — never `Instant.EPOCH`) + `MarkThreadResolvedService`; does NOT touch the parent `package-info.java` files (Plan 03 owns those edges)
