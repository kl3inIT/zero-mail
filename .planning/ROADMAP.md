...
**Plans**: 6 plans
- [ ] 12-W0-oauth-scope-ledger-and-token-store-PLAN.md — `GoogleOAuthScope` enum + source-text scan + `OAuthTokenStore` facade + Liquibase 131-134 schema + ical4j/calendar-API deps.
- [ ] 12-W1-calendar-oauth-and-connection-bootstrap-PLAN.md — `google-calendar` `ClientRegistration` + AuthZ resolver branch + `CalendarOAuthSuccessHandler` + `core.calendar.{domain,exception,persistence,gateway}` entities + `CalendarApiClientFactory`.
...
| 12. Calendar Connection + Triage Foundation | v1.4 | 0/6 | Planned | - |
