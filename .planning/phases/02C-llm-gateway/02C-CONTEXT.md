...
- **D-A2: BYOK path uses per-call `OpenAiApi#mutate()` / `AnthropicApi#mutate()` derive-and-discard.** Gateway `chat(...)` impl: if `tenant_byok_credentials` row exists for current tenant → load + decrypt key + call `parentOpenAiApi.mutate().apiKey(decryptedKey).baseUrl(byokRow.endpoint()).build()` then `parentChatModel.mutate().withApi(derivedApi).build()` then `ChatClient.create(derivedModel)` for that single call; discard after response. Reuses parent's `RestClient` and HTTP infrastructure under the hood — does **not** violate the CLAUDE.md "manually-built ChatClient per request" prohibition (that prohibits hand-rolled `RestClient`/HTTP rebuilds, which Spring AI internals handle). Rationale: only documented M4 seam (`MultiModelService` reference example) for varying both `apiKey` and `baseUrl` per call; survives M4→GA churn since `mutate()` is the public stable seam (issue #477 still open — maintainer-recommended workaround).
- **D-A3: BYOK provider abstraction lives behind a `BYOKChatModelFactory` interface in `core.llm.gateway.springai`.** Two implementations: `OpenAiCompatibleByokFactory` and `AnthropicByokFactory`. Gateway entry resolves: `Optional<TenantByokCredentials> → factory.deriveModel(decryptedKey, endpoint) → ChatClient.create(...)`. Keeps gateway entry-point logic provider-agnostic; ArchUnit ensures both `*Factory` impls are the only places that touch `org.springframework.ai.openai.*` and `org.springframework.ai.anthropic.*` mutate APIs.
...
- **D-G1: BYOK table changeset = `017-tenant-byok-credentials.yaml`.** Phase 2B claimed `014/015/016` (per 2B D-H1). Phase 2C floor is `017`. Schema:
  - `id UUID PRIMARY KEY`
...
- `BYOKChatModelFactory` interface signature (likely `ChatModel deriveModel(byte[] decryptedKey, String endpoint)` but executor can refine)
...
- `backend/core/src/main/resources/db/changelog/changes/` — next-free is `017` (2B claimed `014/015/016`); allocation `017-tenant-byok-credentials.yaml`.
