...
**Gathered:** 2026-05-07
**Updated:** 2026-05-07 (post-research corrections: D-G1 changeset 017→018, D-A2/D-A3 split OpenAI vs Anthropic BYOK seam, D-I5 observation property pin)
**Status:** Ready for planning

<domain>
## Phase Boundary

Phase 2C ships the single `LlmGateway` abstraction on Spring AI 2.0.0-M4 that all LLM traffic must traverse: a `core.llm` Spring Modulith package with a `LlmGateway` interface (planted in `core.llm.model`/`core.llm.service`, with all direct Spring AI usage isolated in `core.llm.gateway.springai`), a 4-step Spring-bean sanitization pipeline (Jsoup → NFC → Unicode-tag-strip → jtokkit truncate ≤3896) returning a `SanitizationContext` carrying token-count + truncation flag for metadata-only observability, defense-in-depth tool-call enforcement (Spring AI ToolCallback + `toolChoice=required` AND post-parse `ActionValidator`) returning `record ToolCallResult(Action action, Map<String,Object> args)`, BYOK with two distinct seam patterns (singleton `ChatClient` + dynamic `ApiKey` reading `TenantContext` for the platform path; per-call `OpenAiApi#mutate()` / `AnthropicApi#mutate()` derive-and-discard for BYOK), encrypted-at-rest `tenant_byok_credentials` table reusing the existing `RefreshTokenCipher` envelope, a `features/llm/` BYOK form in `apps/web` using uncontrolled `<input type="password">` (raw key never enters React state), platform credit-cap wired to Phase 2B `CreditLedger.reserve` with HTTP 402 on insufficient + UI top-up prompt, and a drift-detection scaffold (golden-set + baseline + `@Scheduled` job gated `enabled=false` + CI mock test). UI rendering of credit balance, payment-pending polling, and rules-engine tool-call args parsing are explicit Phase 3+ / Phase 5 territory.
...
- **D-A1: Platform-key path uses singleton `ChatClient` + dynamic `ApiKey` impl reading `TenantContext` ScopedValue.** A `PlatformApiKey` class implementing Spring AI's `ApiKey` interface returns the value from `@ConfigurationProperties("zero-mail.llm.platform").api-key()` on each `getValue()` call (resolved at HTTP send time, not bean construction). One `ChatClient` bean for OpenAI-compatible (default OpenRouter base-url), one for Anthropic if `spring-ai-starter-model-anthropic` is added in v1. Drift detection job + Phase 3 rules compile path + Phase 4 triage path all hit this singleton when no BYOK row exists. Rationale: matches the literal CLAUDE.md "use one client + per-request override" rule, no per-call allocation, simplest to ArchUnit-pin.
- **D-A2: BYOK path is asymmetric per provider (M4 source verified).** OpenAI-compatible BYOK uses per-call `OpenAiApi#mutate().apiKey(decryptedKey).baseUrl(byokRow.endpoint()).build()` then `parentChatModel.mutate().openAiApi(derivedApi).build()` then `ChatClient.create(derivedModel)` for that single call; discard after response. This is the only documented M4 seam for varying both `apiKey` and `baseUrl` per call (`MultiModelService` reference example; issue #477 maintainer-recommended workaround). Anthropic BYOK uses the simpler per-request runtime-options seam: `AnthropicChatOptions.builder().apiKey(decryptedKey).baseUrl(byokRow.endpoint()).model(modelId).build()` passed via `ChatClient.prompt().options(...)` — `AnthropicChatOptions` exposes per-request `.apiKey()` and `.baseUrl()` directly (verified in M4 source); `AnthropicApi.mutate()` is NOT documented in M4. **Do not force symmetry between the two providers** — each uses the seam its SDK exposes. Both seams reuse parent `RestClient` + HTTP infrastructure under the hood, so neither violates the CLAUDE.md "manually-built ChatClient per request" prohibition (which forbids hand-rolled `RestClient`/HTTP rebuilds, not Spring AI internals).
- **D-A3: BYOK provider abstraction lives behind a `BYOKChatModelFactory` interface in `core.llm.gateway.springai`.** Two implementations: `OpenAiCompatibleByokFactory` (uses `OpenAiApi#mutate()` + `OpenAiChatModel#mutate()`) and `AnthropicByokFactory` (uses `AnthropicChatOptions.builder()` per-request options). Each impl picks its own seam internally — the interface signature is uniform (`ChatResponse call(byte[] decryptedKey, String endpoint, String model, String userMessage, List<ToolCallback> toolCallbacks)`) but the implementation strategy differs. Gateway entry resolves: `Optional<TenantByokCredentials> → factory.call(...)`. ArchUnit ensures both `*Factory` impls are the only places that touch `org.springframework.ai.openai.*` and `org.springframework.ai.anthropic.*` per-request override APIs.
...
- **D-C4: `SafetyViolationException` is a `RuntimeException` under `core.llm.model`.** `GlobalExceptionHandler` maps to HTTP 500 + `code=LLM_SAFETY_VIOLATION` (operator-visible signal — should not happen in normal flow; if it does, indicates either model exploit attempt or M4 churn). Not 4xx — this is a programming-/safety-error class. Privacy invariant: exception message MUST NOT contain the rejected action name or any model output content; `event=llm_safety_violation tenantId={} callSite={}` log only.
...
- **D-D3: Two-step Validate-then-Save UX.** Save button is `disabled` until `validateByok.data?.ok === true`. Validate result rendered as a raw shadcn `<Alert variant="success|destructive">` showing `models[]` for OpenAI-compat success, `"Validated successfully"` for Anthropic. On Save success: form reset (provider radio cleared, refs nulled, success toast). On Save failure: error toast, form preserved for retry. Backend `POST /api/llm/byok/validate` returns `{ok, models?, reason?}`; `POST /api/llm/byok` returns `{ok, savedAt}`.
- **D-D4: Raw shadcn primitives — no wrappers.** Composition inside one `<Card>`: `<RadioGroup>` for provider, `<Input type="text">` for endpoint (shown only when provider === 'openai-compatible'), `<Input type="password">` for key, `<Button onClick={validate}>` (Validate) + `<Button onClick={save} disabled>` (Save), `<Alert>` for validate result. Memory rule: rule-of-three not met — no `ByokFormCard` / `ValidationResultAlert` wrappers.
...
- **D-I5: Spring AI observation property pin (M4 verified)**: explicitly pin `spring.ai.chat.client.observations.log-prompt: false` and `spring.ai.chat.client.observations.log-completion: false` in BOTH `backend/api/src/main/resources/application.yml` AND `backend/worker/src/main/resources/application.yml`. The properties were renamed in the 1.1 line that M4 inherits behavior from (`include-prompt` → `log-prompt`, `include-completion` → `log-completion`); both default to `false` but defensive pinning prevents accidental enable on M4→GA migration. Same for `spring.ai.chat.observations.log-prompt` / `log-completion` if the lower-level chat-model observation also exposes the toggle in M4 — verify exact key path at execute-phase via Context7.

### Claude's Discretion

- Exact Spring AI `ApiKey` interface name in M4 (verify via Context7 at plan-phase — research found documented but pre-GA churn possible)
- `OpenAiChatOptions` vs `AnthropicChatOptions` `toolChoice` exact builder method names (verify in M4)
...
- `CLAUDE.md` §Constraints, §Backend Code Style, §Conventions, §"Hard do not use" list — Java 25, Spring Boot 4.0.6, Spring AI 2.0.0-M4, no Lombok, thin controllers + service-owned `@Transactional`, `IdentifiedEnum`/`fromId` fail-loud, privacy log format `event=opaque tenantId={}`, enterprise-readability variable naming, "manually-built ChatClient per request" prohibition, "no Spring AI imports outside one adapter".
...
- `.planning/research/STACK.md` — Spring AI 2.0.0-M4 starter, OpenRouter base-url, Spring AI observation `include-prompt: false / include-completion: false`.
...
- `gradle/libs.versions.toml` — add `springAi = "2.0.0-M4"` version + libraries (`spring-ai-bom`, `spring-ai-starter-model-openai`, optional `spring-ai-starter-model-anthropic`, `jtokkit`).
...
- **Spring AI 2.0.0-M4 reference docs** — `https://docs.spring.io/spring-ai/reference/api/chat/openai-chat.html`, `.../chat/anthropic-chat.html`, `.../chatclient.html` (Multiple OpenAI-Compatible Endpoints `MultiModelService` example), `.../tools.html`. Verify: `OpenAiApi#mutate()` / `AnthropicApi#mutate()` API signatures, `ApiKey` interface shape, `ToolCallback` registration on `ChatClient.prompt()`, `internalToolExecutionEnabled(false)`, `toolChoice` per-provider semantics.
- **Spring AI issue #477** — `https://github.com/spring-projects/spring-ai/issues/477` — Dynamic API Key per request (still open; maintainer-recommended workaround = `mutate()`). Status check at plan-phase.
...
- **Spring AI M4 milestone caveat**: M4 → GA churn is real (M7→M8 silently broke `tools()`). All direct Spring AI usage isolated in `core.llm.gateway.springai/` — ArchUnit enforces. Pin `mutate()` (documented public seam) over interceptors / undocumented internals.
...
- **BYOK custom endpoint user story**: User pastes any OpenAI-compatible URL (Anthropic via OpenRouter, Together.ai, Fireworks, self-hosted vLLM, etc.) into the `OpenAI Compatible` endpoint field. This is why Option A from research (dynamic `ApiKey` only) is insufficient and Option B (`mutate()` covering both `apiKey` and `baseUrl`) is required.
