...
- [ ] `backend/core/src/test/java/com/zeromail/core/arch/LlmGatewayBoundaryTest.java` — ArchUnit rule (M-1 strict, no exemptions): `org.springframework.ai..` outside `core.llm.gateway.springai` → fail; `com.openai.*` / `com.anthropic.*` outside same → fail; `org.jsoup..` / `com.knuddels.jtokkit..` outside `core.llm.gateway.sanitization` → fail
