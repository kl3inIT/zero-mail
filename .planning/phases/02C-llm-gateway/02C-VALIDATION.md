...
created: 2026-05-07
updated: 2026-05-09 (post-execution audit; full backend + BYOK web validation green)
---
...
- [x] `backend/core/src/test/java/com/zeromail/core/llm/gateway/sanitization/SanitizationPipelineWave0Test.java` — enabled by Plan 02 — `pipeline.sanitize("<script>alert(1)</script>hi").content() == "hi"`
- [x] `backend/core/src/test/java/com/zeromail/core/llm/service/LlmGatewayWave0Test.java` — enabled by Plan 03 — gateway returns ToolCallResult under TenantContext-bound mock
- [x] `backend/core/src/test/java/com/zeromail/core/llm/service/ActionValidatorWave0Test.java` — enabled by Plan 04 — `validator.validate("send")` throws SafetyViolationException; `validator.validate("label")` returns Action.LABEL
- [x] `backend/core/src/test/java/com/zeromail/core/arch/LlmGatewayBoundaryTest.java` — strict ArchUnit rule with Spring AI/vendor imports confined to `core.llm.gateway.springai`, jsoup/jtokkit confined to `core.llm.gateway.sanitization`, and no `LlmGatewayImpl` exemption
- [x] `backend/core/src/test/java/com/zeromail/core/llm/persistence/TenantByokCredentialsPersistenceWave0Test.java` — Testcontainers Postgres + UNIQUE-constraint upsert
- [x] `backend/core/src/test/resources/llm/prompt-injection/{html-script-tag,unicode-tag-injection,zero-width-rtl,ignore-previous-instructions,over-budget}.txt` — 5 corpus fixtures
- [x] `backend/core/src/main/resources/llm/golden-set.json` + `golden-baseline.json` — 20-fixture drift sample
...
| Metric | Count |
|--------|-------|
| Task rows audited | 15 |
| Automated rows green | 15 |
| Unresolved gaps found | 0 |
| Resolved by this audit | 0 |
| Escalated/manual-only | 0 |
| Prior UAT gaps confirmed resolved | 1 |

### Audit Evidence

- `02C-UAT.md` is `status: resolved` and links the BYOK validation clarity gap to `02C-GAP-01-SUMMARY.md`.
- `02C-GAP-01-SUMMARY.md` confirms explicit valid-key/configuration copy, green success styling, and component/browser regression tests.
- `rg "@Disabled|Disabled" backend/core/src/test/java/com/zeromail/core/llm backend/worker/src/test/java/com/zeromail/worker/llm backend/api/src/test/java/com/zeromail/api/controllers/llm apps/web/features/llm apps/web/__tests__/byok-key-handling.test.ts apps/web/e2e/byok.spec.ts` found no disabled Java validation tests; matches were only frontend assertions for disabled buttons.

### Commands Run

- `.\gradlew.bat --no-daemon --max-workers=1 :backend:core:test --tests "*Llm*"` — passed in 1m06s.
- `.\gradlew.bat --no-daemon --max-workers=1 :backend:core:test :backend:api:test :backend:worker:test` — passed in 2m37s.
- `pnpm -C apps/web i18n:check` — passed; 374 vi/en leaf keys.
...
- [x] UAT gap closure is covered by automated component + Playwright tests
- [x] No watch-mode flags
- [x] Feedback latency < 120s for quick run
- [x] `nyquist_compliant: true` set in frontmatter (H-3 closed)

**Approval:** Phase 02C is Nyquist-compliant after post-execution audit.
