...
| SET-VOICE-04 | chat-tool + REST share `AssistantKnowledgeService.append` | ArchUnit | `./gradlew :backend:core:test --tests KnowledgeSnippetSingleWriteSiteTest` | ❌ Wave 0 | ⬜ pending |
...
| SET-AI-04 | enum-only response; 401 provider body never leaks; `OK` carries `models[]` capped 100 | mvc slice | `./gradlew :backend:api:test --tests ByokTestConnectionEnumOnlyTest` | ❌ Wave 0 | ⬜ pending |
...
| Whole page | flat-section golden path | Playwright e2e | `pnpm --filter web e2e -- ai-settings.spec.ts` | ❌ Wave 0 | ⬜ pending |
...
- **Frontend e2e stubs:**
  - `apps/web/e2e/ai-settings.spec.ts` (flat-section golden path covering Voice + Behavior + Updates + Safety net + AI Provider Dialogs, plus Knowledge add/edit/delete and BYOK Save → Test → Activate flow)
...
| BYOK round-trip against a real provider | SET-AI-04 happy path | Live API calls aren't run in CI (privacy, cost, flake). Mocked test asserts contract; one real-provider smoke confirms the wire format. | After enabling auto-send rules, set a test OpenAI key against `https://api.openai.com/v1`, click Test (expect `OK` + non-empty `models[]`), click Activate, send a chat message, confirm completion went through BYOK URL via app log. |
...
- [ ] Wave 0 covers all ❌ MISSING references (33 stub files listed)
