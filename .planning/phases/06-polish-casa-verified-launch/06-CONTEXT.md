...
- **D-08:** The Playwright spec drives the journey by issuing real HTTP POSTs to `GmailPubSubController` for the "incoming message" step — the spec uses `request.fetch()` (Playwright's API-context client) rather than `page.route`. This is intentional: Pub/Sub push is a server-to-server flow and never traverses the browser, so `page.route` literally cannot intercept it.
...
- **`backend/api/src/main/java/com/zeromail/api/controllers/gmail/GmailPubSubController.java`** — entry point both for Playwright's `request.fetch()` (D-08) and for k6's load traffic.
