...
- **D-E1: `CallSite` enum extension is additive.** Add `TRIAGE_PLATFORM_LLM` and `TRIAGE_DETERMINISTIC` to `core.billing.model.CallSite`. ArchUnit rule from Phase 2B that locks `CallSite` membership must be updated to allow the new values; no other call sites are added.
