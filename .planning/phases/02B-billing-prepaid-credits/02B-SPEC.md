...
Stand up a `core.billing` package with a single journal-table credit ledger that exposes a `CreditLedger` interface for Phase 2C to consume (`reserve` / `settle` / `release` / `balance`), drives top-ups via a SePay webhook with signature verification + idempotent crediting, runs a 60-second watchdog that releases reserves older than 5 minutes, and rejects insufficient-balance traffic with HTTP 402 — backend-only, no UI pages.

## Background

The codebase has zero billing code today. `backend/core/src/main/java/com/zeromail/core/` contains only `account/`, `gmail/`, `onboarding/`, `tenant/`, `shared/{lang, persistence, privacy}`, and `config/`. No `core.billing` package, no `CreditLedger`, no payment-provider integration, no Stripe/LemonSqueezy/SePay client code. Liquibase changeset numbering reaches `013-tenants-triage-paused.yaml`.
...
5. **SePay top-up webhook with signature verification + idempotent crediting (BILL-01)**: Payment-notification → atomic ledger TOPUP.
   - Current: No payment integration.
   - Target: `POST /api/billing/sepay/webhook` (no Spring Security session auth — protected by SePay HMAC signature header). The endpoint:
     - Reads HMAC-SHA256 signature from the SePay-documented header (e.g., `X-SePay-Signature`); verifies against `SEPAY_WEBHOOK_SECRET` (resolved from VPS deployment secret with `:?` fail-fast — boot fails if env missing, parity with `REFRESH_TOKEN_KEY_BASE64`). Wrong/missing signature → HTTP 401 + `event=sepay_webhook_signature_invalid` (no payload bytes logged).
     - Parses the SePay JSON payload: extracts `transactionId`, `amountVnd` (integer), and `referenceCode` (encodes the `tenantId` — exact format owned by `discuss-phase`).
     - Resolves `tenantId` from `referenceCode`; rejects with HTTP 400 `BILLING_SEPAY_REFERENCE_INVALID` if not parseable or tenant unknown (no detail leaked).
     ...
     - Inserts a `TOPUP` ledger entry with `ref_type='PAYMENT_SEPAY'`, `ref_id=transactionId`, `amount_credits=credits`. Replay protection: the UNIQUE `(ref_type, ref_id, kind)` constraint makes the second delivery a no-op (caught at repository layer, returned as HTTP 200 to ack so SePay stops retrying).
     - Returns HTTP 200 on success and on duplicate (replay-safe ack).
   - Acceptance: WireMock-style test posts a valid signed payload → ledger has one `TOPUP` entry, balance increments. Same payload re-posted → second call returns HTTP 200, ledger still has exactly one `TOPUP` entry. Bad-signature payload → HTTP 401, no ledger entry. Test app boot fails fast when `SEPAY_WEBHOOK_SECRET` is unset (matches Phase 1.5 `REFRESH_TOKEN_KEY_BASE64` fail-fast pattern). No log line during any of these flows contains the raw payload bytes, the signature header value, or the user's bank-account info.
...
**Out of scope:**
- **Refunds / chargeback automation** — admin SQL or dedicated phase later. SePay's MoR posture handles disputes outside our app.
- **Receipt / invoice PDF generation + email** — SePay sends its own confirmation; PDF receipts deferred until Phase 5+ launch hardening.
...
- **Refresh-token-style key rotation drill for `SEPAY_WEBHOOK_SECRET`** — captured in STATE.md Blockers, follows the same deferral as `REFRESH_TOKEN_KEY_BASE64`.
...
- **Rate limiting on the SePay webhook beyond signature check** — SePay-side retry policy + UNIQUE constraint suffice; abuse-rate limit is a future concern.
...
- **Logback scrub coverage**: Webhook handler MUST NOT log the raw SePay payload, signature header value, or any bank-account / phone-number bytes. The Phase 1 `LogScrubFilter` should already cover `prompt=`, `body=`, etc.; extend coverage in this phase only if a new pattern is needed (`signature=`, `payload=`).
- **Deployment secret**: `SEPAY_WEBHOOK_SECRET` resolved from VPS deployment secret only (Docker secret / systemd credential / locked-down env file) with `:?` fail-fast — no plain-env fallback in prod profile, parity with `REFRESH_TOKEN_KEY_BASE64`.
...
- **Integer credits only** — no fractional credits. Rounding loss on TOPUP (e.g., 999 VND with 1000 VND/credit = 0 credits) is logged with `vndLost` only and not auto-refunded in v1 (SePay-side reconciliation handles edge cases).
...
- [ ] `POST /api/billing/sepay/webhook` verifies HMAC-SHA256 signature with `SEPAY_WEBHOOK_SECRET`; bad signature → HTTP 401; missing env at boot → app fails fast.
...
- [ ] No log line in any flow contains the raw SePay payload, signature header value, or bank-account bytes.
...
**SePay-specific risk note:** SePay is a Vietnam-domestic bank-transfer aggregator (QR / virtual-account model) and does not publish a stable global SDK on Maven Central. The webhook integration is HTTP-only (no SDK), which is fine for v1; verifying SePay's exact signature header name, signature algorithm (HMAC-SHA256 vs Ed25519), and payload schema is `discuss-phase` work — `gsd-research-phase` is recommended before planning to read the live SePay docs (https://docs.sepay.vn/ or equivalent). The SPEC locks the *shape* of the integration (signed webhook + idempotency on `transactionId` + VND→credits conversion) without locking SePay's exact wire protocol.

## Interview Log

| Round | Perspective       | Question summary                                              | Decision locked                                                                                                              |
|-------|-------------------|---------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------|
| 1     | Researcher        | Payment provider for credit top-up?                          | **SePay** (Vietnam-domestic bank-transfer MoR; not Stripe / not LemonSqueezy / not deferred)                                 |
...
| 2     | Researcher (cont) | SePay integration depth?                                      | Full webhook + HMAC signature verify + idempotent credit (not scaffold-only, not admin-grant)                                |
