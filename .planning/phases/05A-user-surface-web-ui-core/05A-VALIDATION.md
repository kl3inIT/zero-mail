...
- [ ] `apps/web/features/billing/` — entire feature folder is new (`api/billing-api.ts`, `query-keys.ts`, `hooks/`, `messages.ts`); the `getAuditLog`/`getLedgerHistory`/`useLedgerHistory` gap stubs return an explicit `{unavailable:true}` sentinel (distinct from an empty list); `messages.ts` has NO bank-account-number/bank-name labels and DOES have a ledger-empty + a ledger-unavailable copy pair.
...
- [ ] Vitest spec stubs/real specs: `useBillingBalance.test.tsx` (Plan 01 — incl. the fake-timer ~45s-refetch-actually-fires assertion), `useTopupCreditWatch.test.tsx` (Plan 01). (`useToggleTriagePause.test.tsx` rewrite → Plan 02; `AuditLog.test.tsx` injected-data → Plan 03; `SenderSafetyNetList.test.tsx` → Plan 03; `LedgerTable.test.tsx` injected-data → Plan 04; `messages.contract.test.ts` privacy extension → Plan 05.)
...
- [ ] Extend `apps/web/scripts/check-i18n.ts` `EN_SCAN_FILES` with every new English-literal-bearing component, using the `(protected)/(app)/...` route-group paths for `triage`/`billing`/`settings`/`settings/privacy`/`rules`/`settings`, leaving onboarding under `(protected)/onboarding/...`. Conditionally own `apps/web/package.json`/`pnpm-lock.yaml` if `pnpm dlx shadcn add` adds a runtime dep.
...
4. **Top-up bank-account fields not in `TopupIntentResponse`** — the response carries only `code`/`amountVnd`/`expiresAt`/`qrPayload`; no `accountNumber`/`accountName`/`bankName`/`transferContent` (`accountNumber` in the schema is on `SepayWebhookPayload`). Degradation: the top-up screen shows the VietQR `qrPayload` (QR image + copyable EMV text) + transfer `code` + `amountVnd` + `expiresAt` countdown + "scan this QR" guidance only; separate bank-account/bank-name copyable fields would need a static frontend config constant/env OR a backend change — both out of 5A's frontend-only scope; logged, not added.
...
- [ ] Wave 0 covers all MISSING references (shared states trio, query-key factories, billing feature folder with `{unavailable:true}` gap stubs, Playwright/Vitest stubs, 320px viewport, i18n scan list with route-group paths)
- [ ] No watch-mode flags in any automated command
- [ ] Feedback latency < 120s (quick loop)
- [ ] Backend-surface gaps (1)–(4) resolved-as-flagged in the affected plans + recorded in `05A-GAPS.md`
- [ ] Route-group split (or the documented fallback) verified against Next 16 docs; `EN_SCAN_FILES` reconciled if the fallback changed paths
- [ ] `git diff --exit-code` clean for `apps/web/lib/api/schema.d.ts`, public `/privacy`, and `backend/`
