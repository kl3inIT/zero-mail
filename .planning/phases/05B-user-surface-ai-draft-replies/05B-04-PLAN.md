...
must_haves:
  ...
  artifacts:
    ...
    - path: "backend/core/src/main/java/com/zeromail/core/shared/pagination/KeysetCursor.java"
      provides: "opaque base64 cursor codec with two overloads: encode(Instant,UUID) and encode(Instant,String) — encodes the FULL Instant (epoch-second + nanos), never just epoch-millis; a distinct nullsLast(String id) sentinel encoded with a literal token (not a valid Instant) for the NULLS-LAST tail; decode exposes optional timestamp + raw id string + an isNullsLast flag; fail-loud on malformed input"
  key_links:
...
This plan adds the new `core.shared.pagination` leaf module and the two `core.triage.projection` / `core.thread.projection` sub-packages. The `shared.pagination` edge on the PARENT modules (`core/triage/package-info.java` and `core/thread/package-info.java` `allowedDependencies`) is added by Plan 03 in the same commit that wires the `triage → thread` Modulith edge — this plan must NOT edit those two parent `package-info.java` files. **This plan `depends_on: ["05B-02", "05B-03"]`** (NOT parallel with 03) precisely so 03's parent-`package-info.java` edits are already committed before this plan's projection sub-packages land — `ApplicationModulesTest` would otherwise fail in a parallel run (the sub-package inherits the parent's `allowedDependencies`, and `shared.pagination` must already be listed there). The ROADMAP execution order reflects this: 03 then 04 (04 is not in the same parallelizable group as 03).
...
<task type="auto" tdd="true">
  ...
  <behavior>
    - `KeysetCursor` — a record `(Instant timestamp /* null when isNullsLast */, String id, boolean isNullsLast)` (`id` is the raw key string: a UUID rendered as text for the audit table, the `gmail_thread_id` string for the needs-reply table; the caller knows which). Internal wire format inside the base64url payload: `"v1:" + epochSecond + ":" + nano + ":" + id` for a normal cursor, `"v1:NULLS_LAST::" + id` for the sentinel — i.e. encode the FULL `Instant` (epoch-second AND nanos), NEVER just `toEpochMilli()` (no millisecond-truncation surprise when two rows share a sub-millisecond timestamp), and use a literal `NULLS_LAST` token rather than a magic `Instant` value so a real row with any timestamp can never collide with the sentinel. Static factories:
      - `String encode(Instant timestamp, UUID id)` = `encode(timestamp, id.toString())`
      - `String encode(Instant timestamp, String id)` = base64url-no-padding of `"v1:" + timestamp.getEpochSecond() + ":" + timestamp.getNano() + ":" + id` — split into at most 4 parts on the FIRST three `:` so an id containing `:` survives.
      - `String nullsLast(String id)` = base64url-no-padding of `"v1:NULLS_LAST::" + id` — used only when paging past the `last_classified_at IS NULL` tail (Task 2).
      - `Optional<KeysetCursor> decode(String cursor)` — returns empty for null/blank; throws `IllegalArgumentException` for malformed (bad base64, missing `v1:` prefix, unparseable epoch-second/nano, missing parts) so the controller can 400 it; recognizes the `NULLS_LAST` token → `new KeysetCursor(null, id, true)`. Does NOT validate the id portion as a UUID — `AuditLogQueryService` does `UUID.fromString(cursor.id())` and lets a bad value surface as `IllegalArgumentException` too.
  ...
  <acceptance_criteria>
    - `KeysetCursor.encode(Instant,UUID)` and `encode(Instant,String)` both round-trip through `decode(...)` preserving the FULL `Instant` (epoch-second + nanos — a cursor whose two rows differ only sub-millisecond does not collapse); `nullsLast(id)` round-trips with `isNullsLast == true` and a null timestamp; malformed input → `IllegalArgumentException`; null/blank → empty; an id containing `:` survives the round-trip; a real cursor can never decode to `isNullsLast`
...
<task type="auto" tdd="true">
  ...
  <behavior>
    ...
    - `NeedsReplyInboxQueryService.page(UUID tenantId, NeedsReplyPageQuery query)` — `@Transactional(readOnly=true)`. Ordering is `order by last_classified_at desc nulls last, gmail_thread_id desc` (matching the composite index `idx_thread_reply_status_inbox` (`tenant_id, bucket, resolved, last_classified_at DESC NULLS LAST, gmail_thread_id DESC`) from Plan 00 — the planner of Plan 00 added it precisely for this query, so the page is index-driven, not a scan, even when most rows have a null `last_classified_at`) so rows whose first classify hasn't landed yet (null `last_classified_at`) sort to the bottom; within that null group rows are ordered by `gmail_thread_id desc` alone. Keyset predicate (only when a cursor is decoded) handles the two regions explicitly:
      - If `cursor.isNullsLast()` (we are already paging inside the null-`last_classified_at` tail): predicate is `last_classified_at is null and gmail_thread_id < ?` (the cursor's `id` string).
      - Otherwise (cursor carries a real `Instant`): predicate is `( last_classified_at < ? ) or ( last_classified_at = ? and gmail_thread_id < ? ) or ( last_classified_at is null )` — everything strictly after the cursor row in `(last_classified_at desc nulls last, gmail_thread_id desc)` order, including the entire null tail.
      Base predicates always present: `tenant_id = ?` and the bucket/resolved filter — `(resolvedOnly is true and resolved is true)` OR `(resolvedOnly is false and bucket = ? and resolved is false)`. SQL: `select gmail_thread_id, bucket, has_draft, draft_id, last_classified_at, resolved from thread_reply_status where <base> and <keyset?> order by last_classified_at desc nulls last, gmail_thread_id desc limit ?+1`. When `limit+1` rows return, drop the last; if the last KEPT row has a non-null `last_classified_at` → `nextCursor = KeysetCursor.encode(thatInstant, gmailThreadId)`; if it is null → `nextCursor = KeysetCursor.nullsLast(gmailThreadId)`; else `nextCursor = null`. No `OFFSET`, no `COUNT(*)` for paging.
...
| T-05B-04-06 | Tampering (silent paging bug) | NULLS-LAST sentinel collision (a magic `Instant.EPOCH`) or millisecond-truncated cursor causing duplicate/skipped rows across a page boundary | mitigate | `KeysetCursor` encodes the full `Instant` (epoch-second + nanos), and the NULLS-LAST tail uses a literal `NULLS_LAST` token (`isNullsLast` flag) that no real timestamp can produce — `KeysetCursorTest` asserts both; the page boundary that straddles the null tail is exercised by a `NeedsReplyInboxQueryService` fixture mixing non-null and null `last_classified_at` rows |
</threat_model>
