...
- Persist response: `watch_history_id` (returned baseline), `watch_expires_at` (returned `expiration` ms epoch), `watch_renewed_at = NOW()`. Set `ingestion_health = HEALTHY` if previously `WATCH_UNHEALTHY` and watch succeeded. Only initialize `last_synced_history_id` from the returned baseline when it is NULL (initial registration or reconnect); regular renewal must preserve a non-null cursor so queued/unprocessed history is not skipped.
