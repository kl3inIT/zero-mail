...
- **D-06:** Local-hour matching = **Postgres-side** computation: `WHERE digest_enabled = true AND EXTRACT(HOUR FROM (now() AT TIME ZONE notification_preference.time_zone))::int = digest_send_hour_local`. Postgres `AT TIME ZONE` uses the IANA tzdata it ships with → DST-safe without coupling to JVM `ZoneId`. App layer still uses `ZonedDateTime`/`ZoneId.of(tenant.timeZone)` for the `[hour_local - 24h, hour_local)` window math when composing the digest content.
- **D-07 (REVIEW-AMENDED 2026-05-13, locked v1 — replaces prior wording flagged by OpenCode H1):** Worst-case lateness within the same clock hour is ~59 minutes (acceptable for a daily summary at hour-aligned precision). **HOWEVER:** if the worker is down through a tenant's exact `digest_send_hour_local` tick, that tenant's digest for that day is **SKIPPED with NO catch-up** — the next tick sees `EXTRACT(HOUR FROM (referenceInstant AT TIME ZONE t.time_zone))::int != digest_send_hour_local` for that tenant and does not claim a row. The previous wording incorrectly claimed the 21:00 tick would still send for a tenant with `digest_send_hour_local = 20` if 20:00 was missed; that is impossible under D-06's exact-hour-match query. A retroactive catch-up mode (extend the claim query to also accept tenants where `digest_day_local < today_local` and no SENT row exists for that day) is reserved for v2. The `/settings → Notifications` UI MUST communicate this trade-off (helper text near the send-hour Select). The next-day window math still anchors on the **send moment** (`referenceInstant.atZone(tenant.timeZone)`), shared with the SQL claim query for clock-coherence (Codex C8).
