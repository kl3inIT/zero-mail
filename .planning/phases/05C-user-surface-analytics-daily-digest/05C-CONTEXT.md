...
- **D-07:** Worst-case lateness = ~59 minutes (acceptable for a daily summary at hour-aligned precision). If a tenant's `digest_send_hour_local = 20` and the worker is down 19:30–20:30 local, the 21:00 tick still claims the (tenant, digest-day) row and sends. The next-day window math anchors on the **send moment** (`hour_local` of the run), not on wall-clock 20:00.
