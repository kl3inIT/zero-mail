...
must_haves:
  truths:
    ...
    - "Each per-tenant dispatch runs in its OWN transaction (Propagation.REQUIRES_NEW): tenant N's Resend failure MUST NOT roll back tenants 1..N-1's claim INSERTs / SENT UPDATEs — outer scheduledDispatch() is NOT @Transactional — REVIEW FIX (Codex C6 + OpenCode H2, S1)"
    ...
    - "Idempotency write order: (1) claim INSERT commits, (2) Resend.send() called OUTSIDE any DB transaction, (3) markSent / markFailed runs in its own transaction. Three separate transactional units per tenant — REVIEW FIX (Codex C6 split)"
  ...
  key_links:
    ...
    - from: "DigestDispatchScheduler.dispatchOne"
      to: "@Transactional(propagation = Propagation.REQUIRES_NEW)"
      via: "per-tenant transactional unit — REVIEW FIX Codex C6 / OpenCode H2"
...
<task type="auto" tdd="true">
  ...
  <behavior>
    ...
    - `dispatchOne(DueTenant tenant, Instant referenceInstant)` method (private, separate Spring bean method on the scheduler OR delegated to a `DigestDispatchTenantWorker` collaborator to make `REQUIRES_NEW` work — Spring AOP self-invocation does NOT cross `@Transactional` boundaries on the same bean; delegate to a separate `@Component` bean):
      ...
      - Inside the scope:
        ...
        2. `Instant sendMoment = referenceInstant;` `LocalDate digestDayLocal = referenceInstant.atZone(tenantZone).toLocalDate();` (Codex C8 — Java digest_day_local derived from the SAME referenceInstant that the SQL EXTRACT(HOUR) used).
