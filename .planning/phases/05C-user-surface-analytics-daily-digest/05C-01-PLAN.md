...
<task type="auto" tdd="true">
  ...
  <behavior>
    ...
    - Changeset 033 (REVIEW FIX Codex HIGH-C — Liquibase default shape): `addColumn` `time_zone varchar(64)` to `tenants` with `defaultValue: Asia/Ho_Chi_Minh` (Liquibase `defaultValue` is the correct YAML key for a string literal default — DO NOT use `defaultValueComputed`, which is for SQL expressions like `current_timestamp` and would emit `DEFAULT Asia/Ho_Chi_Minh` unquoted/unparseable) and `constraints: { nullable: false }`; existing rows backfilled by the default at column-add time
...
<task type="auto" tdd="true">
  ...
  <behavior>
    ...
    - Test (`OAuthProvisioningDefaultsTest`): mock or real Testcontainers; after `provisionBundledOAuth(newGoogleSubject, newEmail, ...)` returns, assert (a) one new `tenants` row with `time_zone = 'Asia/Ho_Chi_Minh'`, (b) one new `notification_preference` row `(tenant_id=newTenant, channel='EMAIL', digest_enabled=true, digest_send_hour_local=20)` — uppercase `'EMAIL'` literal per Codex C2 (JPA `@Enumerated(STRING)` stores the enum name; lowercase `'email'` would never appear in the DB), (c) when `notificationPreferenceService.insertDefaults` throws a synthetic exception, the entire transaction rolls back (no tenant row, no user row, no gmail_connection row)
  </behavior>
