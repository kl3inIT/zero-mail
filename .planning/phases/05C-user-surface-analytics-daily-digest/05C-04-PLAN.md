...
must_haves:
  truths:
    ...
    - "WindowChips NEVER passes an empty string to the API — `Tabs.onValueChange` is gated to the 3 valid values; URL state is normalized to `7d` when the URL param is missing or empty (OpenCode L1 frontend-side counterpart)"
    - "Zero-data response renders explicit empty-state copy in every panel — no NaN, no infinite spinner"
    - "/settings shows a Notifications section with a Switch (digest_enabled) + Select (send hour 0-23) + read-only time-zone label + helper text disclosing v1 missed-hour-no-catch-up policy (D-07 locked) — REVIEW FIX (OpenCode H1 user-facing communication)"
    ...
    - "Playwright e2e adds a case asserting `?window=` (empty) and `?window=bogus` (invalid) both render the analytics page cleanly — empty falls back to 7d (200), bogus shows the inherited 400 error UI per Phase 5A error.tsx — REVIEW FIX (OpenCode L1)"
  artifacts:
...
**Window chip URL-param contract (D-21 + UI-SPEC §A):** **REVIEW FIX (OpenCode L1 frontend-side):** the window value flowing from URL → state → API call MUST be normalized through a closed-enum gate. Read `useSearchParams().get('window')` then `(raw == null || raw.trim() === '' || !['7d','30d','90d'].includes(raw.trim())) ? '7d' : raw.trim()`. The hook NEVER sends `?window=` (empty), `?window=bogus`, or `?window=undefined` to the backend; on URL-param invalid, the UI auto-corrects the URL via `router.replace('?window=7d', { scroll: false })` so the canonical state matches what the API saw. The window param is the source of truth — `/analytics?window=30d` is shareable; back/forward navigation works.
...
<task type="auto" tdd="true">
  ...
  <behavior>
    ...
    - `AnalyticsPageClient` is the client component owning the URL-search-param window chip state (`'use client'`). **REVIEW FIX (OpenCode L1)** — implement a small `normalizeWindow(raw: string | null): '7d' | '30d' | '90d'` helper: `const trimmed = raw?.trim(); return trimmed && ['7d','30d','90d'].includes(trimmed) ? trimmed as '7d'|'30d'|'90d' : '7d';`. The client uses `const window = normalizeWindow(searchParams.get('window'))` and, IF the URL contained an invalid raw value (`raw !== null && raw !== window`), calls `router.replace('?window=' + window, { scroll: false })` once on mount via `useEffect` to canonicalize the URL. On chip change: `router.replace('?window=' + newValue, { scroll: false })`. Renders the 4 panels in a Tailwind grid (`grid grid-cols-1 md:grid-cols-2 gap-6`) above the rule-hits panel which is full-width.
    - `WindowChips` renders shadcn `Tabs` with 3 triggers `7d/30d/90d`; `Tabs.value` binds to the normalized window value (never empty, never invalid)
...
<task type="auto" tdd="true">
  ...
  <behavior>
    ...
    - `messages.ts`: every key from UI-SPEC §"Copywriting Contract" /settings table — `settings.notifications.title`, `description`, `toggle.{label,helperOn,helperOff}`, `sendHour.{label,helper,downtimeNote}`, `timeZone.{label,tooltip}`, `toast.{savedTitle,errorTitle,retry}` — both vi + en. **REVIEW FIX (OpenCode H1 user-facing communication):** add `sendHour.downtimeNote` key with copy disclosing the D-07 LOCKED v1 policy:
    ...
    Display this note as small secondary text directly under the Select via `aria-describedby` (so screen readers announce it alongside the field).
    - `NotificationsSection` `'use client'`: renders the section heading + description + Switch (digest_enabled) + Select (0–23 send hour, displayed as `00:00`..`23:00`) + read-only time-zone label with tooltip icon; Switch and Select both fire the mutation `onChange`; Select stays mounted but `disabled` when Switch is OFF (UI-SPEC §"Component Inventory" B); sonner toast on success and error; aria-describedby helper text on Switch + Select. **REVIEW FIX (OpenCode H1)** — under the Select render a `<p id="send-hour-downtime-note" className="text-xs text-muted-foreground">{t('settings.notifications.sendHour.downtimeNote')}</p>` and reference it via `aria-describedby="send-hour-downtime-note"` on the Select trigger.
    ...
    - `e2e/analytics.spec.ts` (Playwright): auth helper from existing e2e setup; visit `/analytics`; assert 4 panels present + window chips visible; click `30d` chip; assert URL is `/analytics?window=30d` and panels re-render; resize viewport to 320px; assert no horizontal scroll + window chips wrap or scroll-x; verify vi locale rendering. **REVIEW FIX (OpenCode L1)** — additional cases: (1) visit `/analytics?window=` (empty) → assert page renders with 7d panels AND the URL is canonicalized to `/analytics?window=7d` by the AnalyticsPageClient mount-time effect; (2) visit `/analytics?window=bogus` → assert page renders with 7d panels AND the URL is canonicalized to `/analytics?window=7d` (frontend gate prevents the backend from ever seeing `bogus`).
    - `e2e/settings-notifications.spec.ts` (Playwright): visit `/settings`; assert Notifications section visible; click Switch → assert toast appears + state persists across reload; change Select to `08:00` → assert persist; verify Switch OFF disables but does not unmount Select; verify vi + en; **REVIEW FIX (OpenCode H1)** — assert the `sendHour.downtimeNote` copy is rendered (the v1 missed-hour policy disclosure) in BOTH vi and en under the Select.
...
- **Window param normalization (OpenCode L1 frontend-side):** `?window=`, `?window=bogus`, `?window=undefined` all render the 7d default and auto-correct the URL to `?window=7d`; the API never receives invalid values from the frontend
...
- **D-07 missed-hour policy disclosure (OpenCode H1):** `sendHour.downtimeNote` copy renders under the Select in both vi + en, referenced by `aria-describedby` on the Select trigger
