...
- Rule authoring policy: natural-language input is only a compiler front-end. The authoritative saved rule must be a structured, editable `When/Then` schema; keep the original user text only as `sourceText`/audit metadata. **Rule-triggered** write actions stay limited to `label`, `archive`, and `save_draft`; rule-triggered auto-send/auto-forward/spam/delete/webhook (= rule fires → action without per-message user click) remain forbidden without a new product/security phase. User-confirmed send/reply/forward via chat assistant preview card (user reviews + clicks per message) is a different pathway, not auto-send, and is allowed under the broader Constraints "Write actions allowed" scope (v1.1+).
