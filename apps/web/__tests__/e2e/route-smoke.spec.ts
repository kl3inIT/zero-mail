 ...
 *
 * /docs and /docs/getting-started are marked `test.fixme` until Plan 06 ships
 * the docs scaffold. They turn into runnable tests in Plan 08 final
 * verification.
 */
...
test.describe('Phase 1.3 route-smoke (REVIEWS Revision 2 — Codex HIGH #5)', () => {
  ...
  for (const route of PLAN_06_PENDING_ROUTES) {
    test(`${route.path} resolves`, async ({ page }) => {
      test.fixme(true, 'Docs scaffold lands in Plan 06 (apps/web/content/docs/*.mdx).');
      const response = await page.goto(route.path, { waitUntil: 'domcontentloaded' });
      expect(response).not.toBeNull();
