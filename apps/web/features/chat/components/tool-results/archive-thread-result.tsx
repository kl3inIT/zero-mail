...
export function ArchiveThreadResult({ input, output }: { input: unknown; output: unknown }) {
  ...
  return (
    <SubtleToolCollapsible title="Đã lưu trữ chuỗi hội thoại">
      <StatusLine icon={MailX}>Đã lưu chuỗi hội thoại này (bỏ khỏi Inbox).</StatusLine>
      {threadId && <OpenInGmailLink href={gmailThreadUrl(threadId)} />}
