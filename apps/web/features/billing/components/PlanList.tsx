...
export function PlanList() {
  ...
  return (
    <div className="space-y-6">
      ...
      >
        {bankTransferIntent && <BankTransferDialog intent={bankTransferIntent} />}
      </Dialog>
...
function BankTransferDialog({ intent }: { intent: BankTransferIntentResponse }) {
  const t = useTranslations();
  ...
  return (
    <DialogContent className="gap-5 p-6 sm:max-w-[380px]">
      ...
      <div className="space-y-3">
        <div className="bg-muted/40 rounded-xl border p-3 text-center">
          <p className="text-muted-foreground text-xs">{t('billing.bankTransfer.amount')}</p>
          <p className="text-xl font-semibold">{formatVnd(intent.amountVnd)}₫</p>
        </div>
        ...
        <p className="text-muted-foreground text-center text-xs">{bankSummary}</p>
        <p className="text-muted-foreground text-center text-xs">
          {t('billing.bankTransfer.expiresAtShort', { time: expiresAt })}
        </p>
      </div>
    </DialogContent>
  );
