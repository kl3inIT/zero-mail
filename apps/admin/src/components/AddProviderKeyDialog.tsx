...
const KEY_FORMAT_OPTIONS: { value: KeyFormat; label: string }[] = [
    ...
    {value: 'GOOGLE_FORMAT', label: 'Google GenAI'},
];

const COMPATIBLE_GATEWAY_FORMAT_OPTIONS = KEY_FORMAT_OPTIONS.filter(
    (entry) => entry.value !== 'GOOGLE_FORMAT',
);
...
const SPRING_AI_BUILT_IN_FORMAT: Record<string, KeyFormat> = {
...
function compatibleFormatOptionsFor(provider: string) {
    const builtInFormat = SPRING_AI_BUILT_IN_FORMAT[provider];
    if (builtInFormat) {
        return KEY_FORMAT_OPTIONS.filter((entry) => entry.value === builtInFormat);
    }
    return COMPATIBLE_GATEWAY_FORMAT_OPTIONS;
}
...
function AddProviderKeyForm({
    ...
    const compatibleFormatOptions = compatibleFormatOptionsFor(provider);
    const [keyFormat, setKeyFormat] = useState<KeyFormat>(
        SPRING_AI_BUILT_IN_FORMAT[provider] ?? 'OPENAI_FORMAT',
    );
    ...
    return (
        <DialogContent className="sm:max-w-xl">
            ...
            >
                ...
                <div className="space-y-2">
                    <Label htmlFor="add-key-format">Kiểu tương thích</Label>
                    <Select
                        value={keyFormat}
                        onValueChange={(next) => {
                            setKeyFormat(next as KeyFormat);
                            setTestResult(null);
                        }}
                        disabled={compatibleFormatOptions.length === 1}
                    >
                        <SelectTrigger id="add-key-format" className="h-10 w-full">
                            <SelectValue/>
                        </SelectTrigger>
                        <SelectContent>
                            {compatibleFormatOptions.map((entry) => (
