...
/**
 * D-03 crash recovery for the {@code processing_job} outbox. Any row with {@code status='RUNNING'}
 * and a {@code heartbeat_at} older than 5 minutes is reset back to {@code status='QUEUED'} with
 * {@code attempts} incremented, so a fresh worker pickup loop can re-claim it via {@code FOR UPDATE
 * SKIP LOCKED}.
 *
...
public class ProcessingJobReaperBatch {
    ...
    /** Reset all stale RUNNING rows to QUEUED + attempts++. Returns the row-count reaped. */
    public int reap() {
