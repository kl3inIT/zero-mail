 ...
 * self-invoked method on the same instance bypasses the AOP interceptor and the database connection
 * runs in autocommit mode. By moving the transactional method onto its own collaborator, the call
 * from {@link CreditReserveWatchdog#tick()} crosses a proxy boundary and the {@code FOR UPDATE SKIP
 * LOCKED} row locks acquired by the scan stay held until the accompanying ledger writes finish.
 *
...
public class CreditReserveWatchdogBatch {
     ...
     * {@code RELEASED} in the same statement, and the wrapping transaction holds those locks until
     * the matching {@code RELEASE} ledger entries are inserted on the same connection.
     *
     * <p>The RELEASE ledger insert uses {@code ON CONFLICT DO NOTHING} against {@code
     * uq_credit_ledger_entry_ref_kind}, so concurrent watchdog invocations or a re-run after a
     * partial commit are both safe — the first writer wins and later writers no-op.
     */
