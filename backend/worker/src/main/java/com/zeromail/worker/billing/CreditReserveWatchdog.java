...
/**
 * Releases orphaned credit reservations after the normal reserve/settle/release path has
...
public class CreditReserveWatchdog {
    ...
    public CreditReserveWatchdog(
        ...
        releasedTotal = Counter.builder("zero_mail.billing.watchdog.released_total")
    ...
    private void releaseOne(StaleReservation staleReservation) {
        ScopedValue.where(TenantContext.TENANT, staleReservation.tenantId().toString()).run(() -> {
            try {
                creditLedger.release(new ReservationId(staleReservation.id()));
                long ageSeconds = Duration.between(staleReservation.createdAt().toInstant(), Instant.now())
                        .toSeconds();
                log.info("event=credit_reserve_released_stale tenantId={} reservationId={} ageSeconds={}",
                        staleReservation.tenantId(), staleReservation.id(), ageSeconds);
