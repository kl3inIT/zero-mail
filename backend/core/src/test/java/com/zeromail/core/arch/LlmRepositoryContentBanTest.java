...
class LlmRepositoryContentBanTest {
    ...
    // apiKey
    // decryptedKey
    private static final Pattern BANNED_METHOD_NAME =
            Pattern.compile(
            ...
            classes()
                    ...
                    .should(
                            new ArchCondition<JavaClass>(
                                ...
                                @Override
                                public void check(
                                        JavaClass repositoryClass, ConditionEvents events) {
                                    for (JavaMethod repositoryMethod :
                                            repositoryClass.getMethods()) {
                                        if (!BANNED_METHOD_NAME
                                                .matcher(repositoryMethod.getName())
                                                .matches()) {
                                            continue;
                                        }
                                        boolean hasStringParameter =
                                                repositoryMethod.getRawParameterTypes().stream()
                                                        .anyMatch(
                                                                parameterType ->
                                                                        parameterType
                                                                                .getName()
