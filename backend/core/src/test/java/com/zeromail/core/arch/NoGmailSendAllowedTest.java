...
class NoGmailSendAllowedTest {
    ...
    @ArchTest
    static final ArchRule no_code_calls_gmail_send_apis = noClasses()
            .should(new ArchCondition<JavaClass>("call Gmail.Users.Messages.send or Gmail.Users.Drafts.send") {
                @Override
                public void check(JavaClass javaClass, ConditionEvents conditionEvents) {
                    javaClass.getMethodCallsFromSelf().forEach(methodCall -> {
                        ...
                        if (!targetOwnerName.endsWith(GMAIL_MESSAGES_OWNER)
                                && !targetOwnerName.endsWith(GMAIL_DRAFTS_OWNER)) {
                            return;
                        }
                        conditionEvents.add(SimpleConditionEvent.violated(
                                methodCall,
                                "Forbidden Gmail send call at " + methodCall.getSourceCodeLocation()));
                    });
                }
            })
            .because("TRG-03: Zero Mail v1 may label, archive, or save drafts, but must never send mail.")
