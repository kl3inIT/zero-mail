...
class RuleEvaluatorTest {
  ...
  void evaluates_every_deterministic_matcher_type_against_preview_safe_input() {
    ...
    assertMatched(new MatcherNode.SubjectEqualsMatcher("subject-equals", "Receipt from Stripe"));
    assertMatched(new MatcherNode.SubjectRegexMatcher("subject-regex", "(?i)^receipt.*stripe$"));
    ...
    assertMatched(new MatcherNode.GmailCategoryPresentMatcher("category-present", "promotions"));
    ...
    assertMatched(
        new MatcherNode.MessageDateMatcher(
            "date-after", MatcherNode.MessageDateOperator.AFTER, LocalDate.parse("2026-05-01")));
    assertMatched(
        new MatcherNode.MessageDateMatcher(
            "date-on", MatcherNode.MessageDateOperator.ON, LocalDate.parse("2026-05-08")));
  ...
  void boolean_groups_apply_tri_state_semantics_in_deterministic_order() {
    ...
    MatcherNode missingMatcher = new MatcherNode.GmailLabelPresentMatcher("missing", "Label_999");
    MatcherNode deferredMatcher =
        new SemanticIntentMatcher("deferred", "messages that need a thoughtful reply", true);
    ...
    RuleEvaluationResult anyWithMatchResult =
        ruleEvaluator.evaluate(
            new MatcherNode.AnyMatcher("any-match", List.of(matchedMatcher, deferredMatcher)),
            ruleEvaluationInput);
    RuleEvaluationResult notMatchedResult =
        ruleEvaluator.evaluate(new MatcherNode.NotMatcher("not", matchedMatcher), ruleEvaluationInput);
    RuleEvaluationResult notDeferredResult =
        ruleEvaluator.evaluate(
            new MatcherNode.NotMatcher("not-deferred", deferredMatcher), ruleEvaluationInput);
  ...
  void semantic_intent_is_deferred_and_rule_evaluator_has_no_llm_gateway_dependency() {
    RuleEvaluationResult evaluationResult =
        ruleEvaluator.evaluate(
            new SemanticIntentMatcher("semantic", "messages that need a thoughtful reply", true),
            matchingInput());
  ...
  void matcher_body_evidence_contract_is_explicit_for_every_phase_3_matcher_type() {
    List<MatcherNode> matcherNodes =
        List.of(
            ...
            new MatcherNode.MessageDateMatcher(
                "date", MatcherNode.MessageDateOperator.ON, LocalDate.parse("2026-05-08")),
            new SemanticIntentMatcher("semantic", "messages that need a thoughtful reply", true));

    assertThat(matcherNodes).allSatisfy(matcherNode -> assertThat(matcherNode.requiresBodyEvidence()).isFalse());
    assertThat(new MatcherNode.AllMatcher("all", matcherNodes).requiresBodyEvidence()).isFalse();
    assertThat(new MatcherNode.AnyMatcher("any", matcherNodes).requiresBodyEvidence()).isFalse();
    assertThat(new MatcherNode.NotMatcher("not", matcherNodes.getFirst()).requiresBodyEvidence()).isFalse();
  }
