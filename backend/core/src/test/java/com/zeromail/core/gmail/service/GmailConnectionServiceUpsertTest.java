...
/**
 * Wave 0 RED scaffold — references {@code GmailConnectionService.upsert(...)} which does
 * NOT exist yet (Plan 02 Task 1 lands it). Compile fails on the {@code service.upsert} call
 * until then.
 *
 * <p>Locks D-A4 (CONTEXT.md): single-row-per-tenant idempotency, version increments on
 * re-grant, scopes/token bytes round-trip cleanly, status transitions
 * {@code DISCONNECTED → CONNECTED} on re-grant and clears {@code disconnectedAt}.
 *
 * <p>Pattern: {@link PostgresContainerTest} (Phase 1.2.1 P03 standard) — real Hibernate
 * round-trip with {@code @TenantId} discriminator + Liquibase audit columns. Tenant binding
 * MUST happen before the JPA session opens (mirrors
 * {@code OAuthProvisioningService.createTenantAndUser} discipline).
 */
