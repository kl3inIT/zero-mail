...
import com.zeromail.core.triage.application.TriageOrchestratorService;
...
class TriagePrivacySweepTest extends PostgresContainerTest {
  ...
  private void configureOpaqueSemanticFailure() {
    when(llmGateway.evaluateSemanticIntents(
        ...
        .thenThrow(
            new LlmEvaluationFailedException(new IllegalStateException(LLM_COMPLETION_SENTINEL)));
  }
            ...
            .setHeaders(
                List.of(
                    new MessagePartHeader()
                        .setName("From")
                        .setValue(SENDER_DISPLAY_NAME_SENTINEL + " <" + RAW_SENDER_EMAIL + ">"),
                    new MessagePartHeader().setName("To").setValue("founder@example.test"),
                    new MessagePartHeader().setName("Subject").setValue(EMAIL_SUBJECT_SENTINEL)));
    return new Message()
  ...
  private void assertAuditRowsAreContentFree(UUID tenantId) {
    ...
    assertThat(auditSurfaces)
        .as("the synthetic triage run should create an audit row for the deterministic archive")
        .hasSize(1);
