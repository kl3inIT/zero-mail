...
/**
 * [BLOCKING] Wave 0 schema-push proof for Phase 1.1: boots Spring against the shared
 * Postgres 17 Testcontainer (so Liquibase auto-applies every changeset including 006-),
 * then asserts via {@code information_schema} that the new {@code users.preferred_language}
 * column has the exact shape locked by CONTEXT.md decision D-B2:
 *
 * <ul>
 *   <li>{@code character varying} type with maximum length 2</li>
 *   <li>{@code NOT NULL}</li>
 *   <li>default value {@code 'vi'}</li>
 *   <li>CHECK constraint enforcing the {@code ('vi','en')} allow-list</li>
 * </ul>
 *
