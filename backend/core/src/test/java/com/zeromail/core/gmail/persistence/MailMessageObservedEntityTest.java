...
class MailMessageObservedEntityTest extends PostgresContainerTest {
    ...
    void insertAndRead_compositePk_roundtrip() {
        ...
        MailMessageObservedEntity entity = new MailMessageObservedEntity(
                tenantId,
                "gmail-1",
                "thread-1",
                123L,
        ...
        ScopedValue.where(TenantContext.TENANT, tenantId.toString()).run(() -> observed.saveAndFlush(entity));

        MailMessageObservedEntity found = ScopedValue.where(TenantContext.TENANT, tenantId.toString())
                .call(() -> observed.findById(new MailMessageObservedId(tenantId, "gmail-1")).orElseThrow());
    ...
    void onConflictDoNothing_deduplication() {
        ...
        int first = observed.insertIfAbsent(tenantId, "gmail-dupe", "thread-dupe", 100L,
                new String[] {"INBOX"}, 1_700_000_000_000L);
        int second = observed.insertIfAbsent(tenantId, "gmail-dupe", "thread-dupe", 101L,
                new String[] {"INBOX"}, 1_700_000_000_001L);
    ...
    void tenantIdFilter_blocksCrossTenantJpaReads() {
        ...
        List<MailMessageObservedEntity> visible = ScopedValue.where(TenantContext.TENANT, tenantA.toString())
                .call(observed::findAll);

        assertThat(visible).extracting(MailMessageObservedEntity::getTenantId)
    ...
    private void insertObserved(UUID tenantId, String messageId, String threadId, String labelsSql) {
        jdbc.update("""
                ...
                """.formatted(labelsSql), tenantId, messageId, threadId, 100L, 1_700_000_000_000L);
    }
}
