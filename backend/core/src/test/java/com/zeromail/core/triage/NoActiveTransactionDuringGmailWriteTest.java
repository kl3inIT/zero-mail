...
class NoActiveTransactionDuringGmailWriteTest extends PostgresContainerTest {
  ...
  @Test
  void gmail_write_phase_suspends_outer_transaction_for_every_gmail_write_type() throws Exception {
    ...
    doAnswer(
        ...
        .when(triageGmailWriter)
        .saveDraft(eq(TENANT_ID), any(TriageActionResult.SaveDraft.class), eq(GMAIL_THREAD_ID));
