 ...
 * {@code com.zeromail.core.shared.persistence.AbstractAuditableEntity} — which then collides
 * with the {@code NOT NULL} audit columns and the per-row {@code defaultValueComputed: now()}
 * does NOT apply (DB defaults only fill columns ABSENT from the INSERT, not those bound to
 * NULL).
 *
 * <p>Mirrors {@code Clock.systemUTC()} from production. Tests that need deterministic time
