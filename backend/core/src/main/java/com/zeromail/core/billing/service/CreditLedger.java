 ...
 * imports verbatim. Callers depend on this interface only; {@link CreditLedgerService} owns the
 ...
 * }</pre>
 *
 * <p>The watchdog ({@code backend/worker.billing.CreditReserveWatchdog}) is the safety net for
 * crashes between {@code reserve} and {@code settle}/{@code release}, not the steady-state
 * finalizer. Reservations older than 5 minutes with no SETTLE/RELEASE journal entry are
 * auto-released by the watchdog.
 *
 * <h3>Idempotency (D-D2 + D-D3)</h3>
 *
 * <ul>
 *   <li>{@code settle} called twice on the same reservation: second call is a no-op.
 *   <li>{@code release} called twice on the same reservation: second call is a no-op.
 *   <li>{@code release} after {@code settle}, or {@code settle} after {@code release}, throws
 *       {@link IllegalLedgerStateException}.
 * </ul>
 *
 ...
 * per-tenant Postgres advisory lock ({@code pg_advisory_xact_lock(hashtext(tenantId))}) inside the
 * same transaction as the SUM-balance check and RESERVE insert.
 *
 * <p>{@link #settle(ReservationId)} and {@link #release(ReservationId)} run in {@code
 * Propagation.REQUIRED}, giving the caller control over finalization atomicity.
 *
 * <h3>BYOK exemption (BILL-07)</h3>
 *
 * <p><b>BYOK traffic bypasses the ledger entirely.</b> Phase 2C's {@code LlmGateway} MUST check the
 * {@code tenant_byok_credentials} table before calling {@link #reserve} and skip this method when a
 * BYOK row exists for the tenant. The {@link CallSite} enum has no BYOK member because BYOK traffic
 * does not enter this interface.
 *
 * <h3>Privacy invariants</h3>
 *
 * <ul>
 *   <li>{@link InsufficientCreditsException} carries no balance number.
 *   <li>Implementation logs use the {@code event=opaque tenantId={}} format.
 * </ul>
 *
 * @see CallSite
 * @see ReservationId
...
public interface CreditLedger {
     ...
     * IllegalLedgerStateException} if the reservation is already RELEASED.
     */
    void settle(ReservationId reservationId);

    /**
     * Reverse a reservation back to available balance. Idempotent on repeat call; throws {@link
     * IllegalLedgerStateException} if the reservation is already SETTLED.
     */
