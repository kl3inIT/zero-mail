...
public class TriageAuditSaga {
    ...
    return switch (command.preWriteIntent()) {
      ...
      case TriageActionResult.Archive ignored -> {
        ...
        yield GmailWriteResult.applied(
            ...
            changeToken(Map.of("operation", "archiveSkipInbox", "removedLabelId", "INBOX")));
      }
      case TriageActionResult.SaveDraft saveDraft -> {
        String draftId =
            triageGmailWriter.saveDraft(command.tenantId(), saveDraft, command.gmailThreadId());
  ...
  public void finalizePhase(UUID tenantId, UUID auditId, GmailWriteResult gmailWriteResult) {
    ...
    if (gmailWriteResult.applied()) {
      triageAuditRepository.markApplied(
          auditId, tenantId, gmailWriteResult.externalRef(), gmailWriteResult.gmailChangeToken());
  ...
  private static String changeToken(Map<String, String> fields) {
    try {
      ...
      throw new IllegalStateException("Unable to serialize Gmail change token", jacksonException);
    }
  }
  ...
  public record GmailWriteResult(
    ...
    public static GmailWriteResult failed(String failureReason) {
      return new GmailWriteResult(false, null, null, requireText(failureReason, "failureReason"));
    }
  }
}
