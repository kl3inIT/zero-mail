...
public class RuleEvaluator {
    ...
    return switch (matcherNode) {
      ...
      case MatcherNode.GmailCategoryPresentMatcher gmailCategoryPresentMatcher ->
          terminal(
              ...
              ruleEvaluationInput.hasGmailCategory(gmailCategoryPresentMatcher.category()),
      ...
      case MatcherNode.GmailCategoryAbsentMatcher gmailCategoryAbsentMatcher ->
          terminal(
              ...
              !ruleEvaluationInput.hasGmailCategory(gmailCategoryAbsentMatcher.category()),
  ...
  private RuleEvaluationResult evaluateSubjectContains(
    ...
    return terminal(
        subjectContainsMatcher.nodeId(), actualSubject.contains(expectedText), "subject_contains");
  }

  private RuleEvaluationResult evaluateSubjectRegex(
      MatcherNode.SubjectRegexMatcher subjectRegexMatcher, RuleEvaluationInput ruleEvaluationInput) {
    Pattern compiledPattern = Pattern.compile(subjectRegexMatcher.regexPattern());
    boolean regexMatched = compiledPattern.matcher(ruleEvaluationInput.sanitizedSubjectExcerpt()).find();
  ...
  private RuleEvaluationResult evaluateMessageAge(
      MatcherNode.MessageAgeMatcher messageAgeMatcher, RuleEvaluationInput ruleEvaluationInput) {
    long messageAgeInDays =
        ChronoUnit.DAYS.between(ruleEvaluationInput.internalDate(), ruleEvaluationInput.observedAt());
    boolean matched =
  ...
  private RuleEvaluationResult evaluateMessageDate(
      MatcherNode.MessageDateMatcher messageDateMatcher, RuleEvaluationInput ruleEvaluationInput) {
  ...
  private RuleEvaluationResult evaluateRecipient(
      String matcherNodeId, String expectedEmail, java.util.List<String> actualRecipients, String reason) {
    String normalizedExpectedEmail = normalized(expectedEmail);
    boolean matched =
        actualRecipients.stream()
            .anyMatch(actualRecipient -> normalized(actualRecipient).equals(normalizedExpectedEmail));
  ...
  private RuleEvaluationResult evaluateTextEquals(
    ...
    return terminal(matcherNodeId, normalized(actualText).equals(normalized(expectedText)), reason);
  }
