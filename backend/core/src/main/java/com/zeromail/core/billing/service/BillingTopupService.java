...
public class BillingTopupService {
  ...
  public BillingTopupIntentEntity createIntent(UUID tenantId, long amountVnd) {
    ...
    int pendingCount =
        intentRepository.countByTenantIdAndStatus(tenantId, BillingTopupIntentStatus.PENDING);
    ...
    String code =
        topupCodeGenerator.generateUniqueCode(
            candidateCode -> intentRepository.findTenantLookupByCode(candidateCode).isEmpty(), 3);
                ...
                transactionTemplate.executeWithoutResult(
                    transactionStatus ->
                        applyTopupCreditTransactional(
                            lookup.id(), sepayTransactionId, transferAmountVnd)));
  }
