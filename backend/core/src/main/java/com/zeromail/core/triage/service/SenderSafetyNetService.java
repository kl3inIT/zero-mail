...
public class SenderSafetyNetService {
  ...
  SenderSafetyNetService(
    ...
    this.tenantProtectedSenderObservationRepository = tenantProtectedSenderObservationRepository;
  ...
  public boolean isProtected(UUID tenantId, String senderEmail) {
    ...
    String senderEmailHash = senderEmailCanonicalizer.redisCacheKeyComponent(canonicalSenderEmail);
    ...
    boolean protectedFlag = queryGmailSentHistory(tenantId, canonicalSenderEmail, senderEmailHash);
  ...
  public void optInSender(UUID tenantId, String senderEmail) {
    ...
    log.info(
        "event=triage_sender_opt_in tenantId={} senderEmailHash={}", tenantId, senderEmailHash);
  }
  ...
  public List<ProtectedSenderListItem> listProtectedSenders(UUID tenantId) {
    ...
    return tenantProtectedSenderObservationRepository.findByTenantId(tenantId).stream()
        .sorted(Comparator.comparing(TenantProtectedSenderObservationEntity::getSenderEmail))
        .map(
            protectedSenderObservation ->
                new ProtectedSenderListItem(
                    protectedSenderObservation.getSenderEmail(),
                    optedInSenderEmails.contains(protectedSenderObservation.getSenderEmail())))
        .toList();
  }
    ...
    try {
      ...
      ListMessagesResponse response =
          gmail
              ...
              .setQ(
                  "in:sent to:"
                      + senderEmailCanonicalizer.gmailSearchToken(canonicalSenderEmail)
                      + " newer_than:90d")
              .setMaxResults(3L)
              .execute();
      List<Message> sentMessages = response.getMessages();
      return sentMessages != null && sentMessages.size() >= 3;
    } catch (InvalidGrantException | IllegalStateException | IOException senderSafetyNetLookupFailure) {
  ...
  private Optional<Boolean> readCachedProtectedFlag(UUID tenantId, String senderEmailHash) {
    ...
    try {
      return Optional.ofNullable(
              stringRedisTemplate.opsForValue().get(cacheKey(tenantId, senderEmailHash)))
  ...
  private void writeCachedProtectedFlag(
    ...
    try {
      stringRedisTemplate
          .opsForValue()
          .set(cacheKey(tenantId, senderEmailHash), Boolean.toString(protectedFlag), CACHE_TTL);
    } catch (RuntimeException redisException) {
      log.debug(
          "event=triage_sender_safety_net_cache_unavailable tenantId={} senderEmailHash={}",
          tenantId,
