...
/**
 * Thrown when the request is authenticated and tenant-bound, but no user row exists for the
 * current tenant. This is an authentication/identity-consistency failure (the session refers
 * to a user that has been deleted or was never provisioned), not a business conflict —
 * GlobalExceptionHandler maps it to HTTP 401, not 409.
 *
