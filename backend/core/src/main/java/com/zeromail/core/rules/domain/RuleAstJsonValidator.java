...
public class RuleAstJsonValidator {
  ...
  private static void validateMatcherNode(
    ...
    switch (matcherType) {
      ...
      case SUBJECT_REGEX -> {
        rejectUnknownFields(matcherNode, rootMatcher, Set.of("regexPattern", "regex", "text"));
        String regexPattern =
            requiredFirstText(matcherNode, MAX_REGEX_LENGTH, "regexPattern", "regex", "text");
      ...
      case NOT -> {
        ...
        validateMatcherNode(required(matcherNode, "child"), false, depth + 1, validationState);
      }
      case SEMANTIC_INTENT -> {
        rejectUnknownFields(matcherNode, rootMatcher, Set.of("intent", "description", "deferred"));
