 ...
 * mechanism via Spring Data + {@link AuditingEntityListener} + the API module's {@code
 * JpaAuditingConfig}).</b>
 *
 ...
 * is needed, set {@code updated_at = NOW()} explicitly in the query, or migrate to a DB trigger for
 * that specific table. This is documented again in {@code
 * core.shared.persistence/package-info.java}.
 *
 * <p><b>Privacy (D-A4):</b> {@code @CreatedBy} / {@code @LastModifiedBy} are deliberately omitted.
 * Row-level user identity is forbidden by the project privacy constraint until the Phase 6
 * audit-log table lands with a proper {@code AuditorAware<UUID>} bean.
 */
