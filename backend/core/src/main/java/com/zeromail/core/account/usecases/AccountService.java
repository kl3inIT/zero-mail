...
public class AccountService {
    ...
    /**
     * Returns the canonical current-user projection for the tenant, or throws {@link
     * CurrentUserNotFoundException} if the session is bound to a tenant whose user row no longer
     * exists. Returns a {@link CurrentUserProjection} record so controllers do not need to import
     * the persistence-managed entity type.
     */
    ...
    /**
     * Persists the caller's preferred language ({@code 'vi'} or {@code 'en'}). The argument is
     * assumed to have already passed Bean Validation ({@code @Pattern(regexp = "vi|en")} on {@code
     * UpdateLanguageRequest}); this method does NOT re-validate the allow-list, because
     * controller-side validation is the single source of truth for that contract.
     *
     * <p>Goes through JPA dirty-tracking on the managed {@link
     * com.zeromail.core.account.persistence.UserEntity} — no raw JDBC, no native UPDATE — so
     * Hibernate's {@code @TenantId} filter and the shared-schema multi-tenant invariants stay
     * honored end-to-end. Cross-tenant writes are impossible: the lookup itself is tenant-scoped
     * via {@code findFirstByTenantId}.
     *
    ...
    /**
     * Deletes the User row for the current tenant. Single-domain delete only — the multi-domain
     * orchestration (gmail connection, onboarding selections, tenant) lives in
     ...
     * state-machine forward-only invariant is enforced inside {@code UserEntity#advanceTo}.
     *
     * @throws CurrentUserNotFoundException if no user row exists for the tenant
     */
    @Transactional
