...
public class RuleCompileResultValidator {
  ...
  public RuleCompileResult validate(
    ...
    try {
      ...
      RuleLanguage sourceLanguage =
          resolveSourceLanguage(detectedLanguage, stringField(arguments, "sourceLanguage", true));
      ...
      if (clarificationRequired || !hasRequiredCompileSlots) {
        return clarificationResult(sourceLanguage, optionalString(arguments, "clarificationQuestion"));
      }

      RuleSchemaVersion schemaVersion = RuleSchemaVersion.fromId(stringField(arguments, "schemaVersion", false));
      String displayName =
  ...
  private String sanitizeClarificationQuestion(RuleLanguage sourceLanguage, String rawQuestion) {
    String question = rawQuestion == null || rawQuestion.isBlank() ? fallbackQuestion(sourceLanguage) : rawQuestion;
    ...
    long questionMarkCount = question.chars().filter(character -> character == '?' || character == '？').count();
    ...
    if (sourceLanguage == RuleLanguage.VI && !containsVietnameseSignal(question.toLowerCase(Locale.ROOT))) {
      return fallbackQuestion(RuleLanguage.VI);
    }
    if (sourceLanguage == RuleLanguage.EN && containsVietnameseSignal(question.toLowerCase(Locale.ROOT))) {
  ...
  private ParsedMatcher parseMatcher(
      ...
      boolean rootMatcher) {
    MatcherType matcherType = MatcherType.fromId(stringField(matcherArguments, "type", "matcherType"));
    ...
    MatcherNode typedMatcher =
        switch (matcherType) {
          case SENDER_EMAIL -> {
            ...
            String email = boundedStringField(matcherArguments, "email", MAX_MATCHER_TEXT_LENGTH, false);
          ...
          case SENDER_DOMAIN -> {
            ...
            String domain = boundedStringField(matcherArguments, "domain", MAX_MATCHER_TEXT_LENGTH, false);
          ...
          case SUBJECT_CONTAINS -> {
            ...
            String text = firstBoundedString(matcherArguments, MAX_MATCHER_TEXT_LENGTH, "text", "value");
          ...
          case SUBJECT_REGEX -> {
            rejectUnknownFields(matcherArguments, fields("regexPattern", "regex", "text"), matcherPath);
            String regexPattern =
                firstBoundedString(matcherArguments, MAX_REGEX_LENGTH, "regexPattern", "regex", "text");
          ...
          case GMAIL_LABEL_PRESENT -> {
            rejectUnknownFields(matcherArguments, fields("labelId", "label"), matcherPath);
            String labelId = firstBoundedString(matcherArguments, MAX_MATCHER_TEXT_LENGTH, "labelId", "label");
          ...
          case GMAIL_CATEGORY_PRESENT -> {
            ...
            String category =
                boundedStringField(matcherArguments, "category", MAX_MATCHER_TEXT_LENGTH, false);
          ...
          case MESSAGE_AGE -> {
            rejectUnknownFields(matcherArguments, fields("operator", "days"), matcherPath);
            MatcherNode.MessageAgeOperator operator =
                MatcherNode.MessageAgeOperator.valueOf(stringField(matcherArguments, "operator", false));
          ...
          case MESSAGE_DATE -> {
            rejectUnknownFields(matcherArguments, fields("operator", "date"), matcherPath);
            MatcherNode.MessageDateOperator operator =
                MatcherNode.MessageDateOperator.valueOf(stringField(matcherArguments, "operator", false));
            LocalDate date = LocalDate.parse(stringField(matcherArguments, "date", false));
            normalizedMatcher.put("operator", operator.name());
          ...
          case ALL -> {
            ...
            List<ParsedMatcher> childMatchers =
                parseChildren(listField(matcherArguments, "children"), matcherPath, schemaVersion);
            List<MatcherNode> children = childMatchers.stream().map(ParsedMatcher::typedMatcher).toList();
            normalizedMatcher.put(
                "children", childMatchers.stream().map(ParsedMatcher::normalizedMatcher).toList());
          ...
          case NOT -> {
            ...
            ParsedMatcher childMatcher =
                parseMatcher(mapField(matcherArguments, "child"), matcherPath + ".child", schemaVersion, false);
          ...
          case SEMANTIC_INTENT -> {
            rejectUnknownFields(matcherArguments, fields("intent", "description", "deferred"), matcherPath);
            boolean deferred = booleanField(matcherArguments, "deferred", false);
            String intent =
                firstBoundedString(matcherArguments, MAX_MATCHER_TEXT_LENGTH, "intent", "description");
  ...
  private List<ParsedMatcher> parseChildren(
    ...
    for (int childIndex = 0; childIndex < childArguments.size(); childIndex++) {
      parsedChildren.add(
          parseMatcher(
              copyStringKeyedMap(childArguments.get(childIndex), matcherPath + ".children"),
  ...
  private List<Map<String, Object>> normalizeActionIntents(List<Object> actionArguments) {
    ...
    for (Object actionArgument : actionArguments) {
      ...
      RuleActionType actionType = RuleActionType.fromId(stringField(actionMap, "type", false));
      ...
      ActionIntent typedAction =
          switch (actionType) {
            case LABEL -> {
              String labelName =
                  firstBoundedString(actionMap, MAX_ACTION_TEXT_LENGTH, "labelName", "value");
              normalizedAction.put("labelName", labelName);
              yield new ActionIntent.Label(labelName);
            }
            case ARCHIVE -> {
              if (actionMap.size() > 1) {
                throw new IllegalArgumentException("archive action cannot carry content");
              }
              yield new ActionIntent.Archive();
            }
            case SAVE_DRAFT -> {
              String instruction =
                  firstBoundedString(actionMap, MAX_ACTION_TEXT_LENGTH, "instruction", "body", "value");
  ...
  private static boolean containsVietnameseSignal(String normalizedText) {
    if (normalizedText.matches(".*[ăâđêôơưáàảãạấầẩẫậắằẳẵặéèẻẽẹếềểễệíìỉĩịóòỏõọốồổỗộớờởỡợúùủũụứừửữựýỳỷỹỵ].*")) {
      return true;
    }
    for (String vietnameseToken :
        List.of(" hoa don ", " hóa đơn ", " bien lai ", " biên lai ", " tu ", " từ ", " nhan ", " nhãn ", " thu ", " thư ", " cua ", " của ", " khong ", " không ")) {
  ...
  private static String stringField(Map<String, Object> values, String primaryFieldName, String fallbackFieldName) {
  ...
  private static String firstBoundedString(
      Map<String, Object> values, int maxLength, String firstFieldName, String... otherFieldNames) {
  ...
  private static String writeJson(Object value) {
    try {
      ...
      throw new IllegalArgumentException("Unable to serialize compile result", serializationFailure);
    }
  }
