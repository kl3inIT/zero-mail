...
public class GoogleOAuthSuccessHandler extends SimpleUrlAuthenticationSuccessHandler {
  ...
  public void onAuthenticationSuccess(
    ...
    String email =
        Objects.requireNonNull(
            oidcUser.getClaimAsString(StandardClaimNames.EMAIL), "OIDC email is required");

    // Load the authorized client once (all checks reuse this single load).
    OAuth2AuthorizedClient authorizedClient =
        authorizedClientService.loadAuthorizedClient("google", authenticationToken.getName());
    ...
    if (authorizedClient == null
      ...
      throw new OAuth2AuthenticationException(
          new OAuth2Error("gmail_scope_required", "Required Gmail scope was not granted", null));
    }
    ...
    boolean settingsBasicMissing =
        !authorizedClient.getAccessToken().getScopes().contains(OAuthScopes.GMAIL_SETTINGS_BASIC);
