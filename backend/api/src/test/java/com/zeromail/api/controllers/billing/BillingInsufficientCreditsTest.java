...
@ActiveProfiles("test")
@Import({TestSessionSupport.class, BillingInsufficientCreditsTest.BillingReserveProbeController.class})
class BillingInsufficientCreditsTest extends ApiPostgresTestBase {
    ...
    void insufficient_balance_returns_402_no_balance_leak() throws Exception {
        ...
        assertThat(responseJson.path("code").asText()).isEqualTo("error.billing.insufficient");
        assertThat(responseJson.path("params").isObject()).isTrue();
        assertThat(responseJson.path("params").isEmpty()).isTrue();
