...
/**
 * Locks the wire shape of the Phase 1.1 ProblemDetail extension contract (CONTEXT.md decisions
 * D-C1, D-C2, D-C3) and the supporting class hierarchy invariant (RESEARCH.md Pitfall 1).
 *
 * <p>Exercises three independent invariants:
 *
 * <ol>
 *   <li>Validation errors flow through {@code handleMethodArgumentNotValid} and emit {@code
 *       application/problem+json} carrying top-level {@code code = "error.validation"} plus a
 *       {@code fieldErrors[]} array shaped {@code {field, code, params}}.
 *   <li>{@link GlobalExceptionHandler} extends Spring's {@link
 *       org.springframework.web.servlet.mvc.method.annotation.ResponseEntityExceptionHandler},
 *       which is mandatory for Bean Validation handlers to fire under {@code
 *       spring.mvc.problemdetails.enabled=true} (mitigates threat T-1.1.02-02).
 *   <li>Existing Phase 1 handlers emit dotted D-C3 codes (e.g., {@code error.auth.forbidden})
 *       instead of the legacy flat keys.
 * </ol>
 *
 * <p>Privacy invariant: the validation test seeds a sentinel value ({@code
 * <<SENTINEL_USER_INPUT>>}) into the rejected field and asserts the sentinel never appears in the
 * response body — the {@link com.zeromail.api.error.AllowedParamScalars} filter must drop it from
 * {@code params}, and the handler must not leak it through {@code title} / {@code detail}.
 */
@ActiveProfiles("test")
...
class ProblemDetailContractTest extends ApiPostgresTestBase {
    ...
    void validationError_returns_problemJson_with_top_level_code_and_fieldErrors()
        ...
        assertThat(responseBody)
                .as(
                        "AllowedParamScalars must filter the sentinel out of params; "
        ...
        assertThat(json.path("message").asString())
                .isEqualTo("error.validation"); // transitional alias
    ...
    void globalExceptionHandler_extends_ResponseEntityExceptionHandler() throws Exception {
        Class<?> superClass =
                Class.forName("com.zeromail.api.config.GlobalExceptionHandler").getSuperclass();

        assertThat(superClass.getName())
                .as(
                        "Mitigates threat T-1.1.02-02 / Spring issue #35982 — without this superclass, "
     ...
     *
     * <ul>
     *   <li>{@code POST /test/validate-locale} — bound to {@link TestLocaleBody}, fires {@code
     *       MethodArgumentNotValidException} when {@code language} fails the {@code vi|en} pattern.
     *   <li>{@code GET /test/throw-access-denied} — throws {@link AccessDeniedException} so the
     *       existing {@code AccessDeniedException} handler runs end-to-end.
     * </ul>
     *
     * Decoupled from Plan 04's {@code PATCH /me/language} so this test does not depend on a
     * downstream plan being landed.
     */
    @TestConfiguration
