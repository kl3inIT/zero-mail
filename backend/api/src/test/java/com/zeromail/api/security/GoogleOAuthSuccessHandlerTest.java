...
/**
 * Phase 01.5: updated to match the bundled-OAuth handler contract.
 * {@link GoogleOAuthSuccessHandler} now receives authorizedClients + users + provisioning.
 */
class GoogleOAuthSuccessHandlerTest {
    ...
    void redirects_to_frontend_onboarding_after_successful_bundled_grant() throws Exception {
        ...
        var handler = new GoogleOAuthSuccessHandler(provisioning, authorizedClients, userRepo, PROPS);
        ...
        var idToken = new OidcIdToken("test-token", Instant.now(), Instant.now().plusSeconds(3600), claims);
        var oidc = new DefaultOidcUser(java.util.List.of(new SimpleGrantedAuthority("OIDC_USER")), idToken);
        ...
        var registration = ClientRegistration.withRegistrationId("google")
                ...
                .scope(Set.of("openid", "profile", "email",
                        OAuthScopes.GMAIL_MODIFY, OAuthScopes.GMAIL_SETTINGS_BASIC))
                .authorizationUri("https://accounts.google.com/o/oauth2/v2/auth")
                .tokenUri("https://oauth2.googleapis.com/token")
                .build();
        var accessToken = new OAuth2AccessToken(
                OAuth2AccessToken.TokenType.BEARER, "fake-at", Instant.now(), Instant.now().plusSeconds(3600),
                Set.of("openid", "profile", "email", OAuthScopes.GMAIL_MODIFY, OAuthScopes.GMAIL_SETTINGS_BASIC));
        ...
        when(authorizedClients.loadAuthorizedClient(eq("google"), anyString()))
                .thenReturn(new OAuth2AuthorizedClient(registration, token.getName(), accessToken, refreshToken));

        when(provisioning.provisionBundledOAuth(anyString(), anyString(), anyString(), anyString()))
                .thenReturn(new OAuthProvisioningService.BundledProvisioningResult(
        ...
        verify(provisioning).provisionBundledOAuth(eq(subject), eq(email), anyString(), anyString());
