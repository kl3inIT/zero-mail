...
/**
 * Wave 0 RED scaffold — references {@link GoogleOAuthSuccessHandler} (extended in Task 2)
 * and {@link BundledGoogleOAuthIntegrationTest.MockAuthorizedClientServiceConfig} harness.
 *
 ...
 *       onboarding_step = GMAIL_CONNECTED.</li>
 *   <li>Scope-missing (no gmail.modify) — no DB rows, 302 to /login?error=gmail_scope_required.</li>
 *   <li>HIGH-1: gmail upsert failure rolls back user + tenant (atomicity invariant).</li>
 *   <li>MED-3: null refresh token on first login — no DB rows, 302 to /login?error=consent_denied.</li>
 *   <li>INFO-7: gmail.settings.basic only missing — provisioning succeeds with warning log.</li>
 * </ol>
 *
 * <p>Privacy: all fixture identifiers use obviously-fake prefixes per threat T-01.5-01-03.
 */
@ActiveProfiles("test")
@Import({TestSessionSupport.class, BundledGoogleOAuthIntegrationTest.MockAuthorizedClientServiceConfig.class})
class BundledGoogleOAuthIntegrationTest extends ApiPostgresTestBase {
    ...
    @Test
    void bundled_oauth_persists_user_tenant_and_gmail_connection_atomically_on_full_grant() throws Exception {
    ...
    void scope_missing_short_circuits_before_provisioning_no_db_write() throws Exception {
        ...
        stubAuthorizedClient(token, FAKE_REFRESH_TOKEN,
                Set.of("openid", "profile", "email", "https://www.googleapis.com/auth/gmail.settings.basic"));
    ...
    void gmail_upsert_failure_rolls_back_user_and_tenant() throws Exception {
        ...
        doThrow(new RuntimeException("Simulated cipher/upsert failure for HIGH-1 atomicity test"))
                .when(gmailConnectionService).upsert(
    ...
    void gmail_settings_basic_only_missing_succeeds_with_warning() throws Exception {
        ...
        stubAuthorizedClient(token, FAKE_REFRESH_TOKEN,
                Set.of("openid", "profile", "email", "https://www.googleapis.com/auth/gmail.modify"));
    ...
    private OAuth2AuthenticationToken buildToken(String subject, String email) {
        ...
        var principal = new DefaultOidcUser(
                ...
                idToken);
        return new OAuth2AuthenticationToken(principal, principal.getAuthorities(), GOOGLE_REGISTRATION_ID);
    }
    ...
    private void stubAuthorizedClient(OAuth2AuthenticationToken token, String refreshTokenValue, Set<String> scopes) {
    ...
    private void stubAuthorizedClientNoRefreshToken(OAuth2AuthenticationToken token, Set<String> scopes) {
        ClientRegistration registration = ClientRegistration.withRegistrationId(GOOGLE_REGISTRATION_ID)
                .clientId("test-google-client")
                .clientSecret("test-google-secret")
                .clientName("google")
        ...
        OAuth2AuthorizedClient client = new OAuth2AuthorizedClient(
                registration, token.getName(), accessToken, null);
        when(authorizedClientService.loadAuthorizedClient(GOOGLE_REGISTRATION_ID, token.getName()))
                .thenReturn(client);
    }
}
