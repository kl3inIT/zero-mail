...
class GmailPubSubControllerIntegrationTest extends ApiPostgresTestBase {
    ...
    static void pubsubProps(DynamicPropertyRegistry r) {
        r.add("zero-mail.api.gmail.pubsub.push-audience-url", () -> AUDIENCE);
        r.add("zero-mail.api.gmail.pubsub.sa-principal-email", () -> SERVICE_ACCOUNT_EMAIL);
        r.add("zero-mail.api.gmail.pubsub.oidc-certificates-url", OIDC::jwksUrl);
    }
    ...
    void missingAuthHeader_returns401() {
        ResponseEntity<String> response =
                post(null, pushEnvelope("msg-missing-auth", "known@example.test", 10L));
    ...
    void validPush_unknownEmail_returns200_dropsSilently() {
        ...
        ResponseEntity<String> response =
                post(validToken(), pushEnvelope(messageId, "unknown@example.test", 101L));
    ...
    void missingMessageId_returns200_noPubSubDeliveryRow() {
        ...
        String data =
                Base64.getEncoder()
                        .encodeToString(
                                ("{\"emailAddress\":\"" + email + "\",\"historyId\":\"103\"}")
    ...
    private static String pushEnvelope(String messageId, String email, long historyId) {
        String dataJson =
                "{\"emailAddress\":\"" + email + "\",\"historyId\":\"" + historyId + "\"}";
        String data = Base64.getEncoder().encodeToString(dataJson.getBytes(StandardCharsets.UTF_8));
