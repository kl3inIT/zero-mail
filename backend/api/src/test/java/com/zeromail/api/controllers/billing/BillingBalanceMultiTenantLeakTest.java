...
class BillingBalanceMultiTenantLeakTest extends ApiPostgresTestBase {
    ...
    void concurrent_virtual_thread_balance_requests_never_cross_tenant() throws Exception {
        List<Seed> seeds = IntStream.range(0, 10)
                .mapToObj(seedIndex -> seedTenant("billing-leak-" + seedIndex, (seedIndex + 1) * 10))
                .toList();
        RestClient client = RestClient.create("http://localhost:" + port);

        try (var taskScope = StructuredTaskScope.<BillingBalanceResponse>open()) {
            var balanceTasks = seeds.stream()
            ...
            for (int seedIndex = 0; seedIndex < seeds.size(); seedIndex++) {
                ...
                assertThat(observedResponse.availableCredits()).isEqualTo(seeds.get(seedIndex).expectedCredits());
    ...
    private Seed seedTenant(String label, int credits) {
        ...
        ScopedValue.where(TenantContext.TENANT, tenantId.toString()).run(() -> {
            userRepository.save(new UserEntity(UUID.randomUUID(), tenantId, googleSubject, email));
            creditLedgerEntryRepository.saveAndFlush(CreditLedgerEntryEntity.topup(
                    UUID.randomUUID(), tenantId, credits, "SEPAY-LEAK-" + tenantId));
        });
