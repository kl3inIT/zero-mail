 ...
 * filter chain, not via MockMvc's standalone setup). Each test seeds its own tenant + user rows so
 * they can run in any order and never share state.
 *
 ...
 * </ol>
 *
 * <p>Privacy: no raw email content, no LLM I/O, no refresh-token shapes appear in any fixture.
 * Sentinels use the obvious-marker convention (e.g., {@code "zz"} for the disallowed locale).
 */
@ActiveProfiles("test")
...
class MeLanguageIntegrationTest extends ApiPostgresTestBase {
    ...
    void patchMeLanguage_persistsToDb_andReturnsUpdated() throws Exception {
        ...
        // Direct DB assertion — bypasses the application stack to prove the column was actually
        // written.
        String dbValue =
                jdbc.queryForObject(
    ...
    void patchMeLanguage_invalidValue_returns400_with_validation_code_and_fieldError()
        ...
        // Privacy / T-1.1.04-04: response must NOT echo Java exception class names or framework
        // prefixes.
        assertThat(body)
