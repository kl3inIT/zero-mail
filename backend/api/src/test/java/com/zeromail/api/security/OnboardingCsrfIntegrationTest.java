...
class OnboardingCsrfIntegrationTest extends ApiPostgresTestBase {
    ...
    void select_template_requires_csrf_token() throws Exception {
        ...
        mvc.perform(post("/onboarding/select-template")
                        .with(oidcLogin().idToken(token -> token
                                .claim("sub", seed.googleSubject())
                                .claim("email", seed.email())))
    ...
    void select_template_accepts_spa_xsrf_cookie_and_header() throws Exception {
        ...
        MockHttpServletResponse tokenResponse = mvc.perform(get("/me")
                        .with(oidcLogin().idToken(token -> token
                                .claim("sub", seed.googleSubject())
                                .claim("email", seed.email()))))
        ...
        ScopedValue.where(TenantContext.TENANT, seed.tenantId().toString()).run(() ->
                assertThat(selections.findByTenantId(seed.tenantId()))
                        .singleElement()
                        .satisfies(selection -> assertThat(selection.getTemplateKey()).isEqualTo("archive-receipts")));
    }
